...
//********************************************************//
double HungarianAlgorithm::Solve(vector <vector<double> > DistMatrix, vector<int>& Assignment) // Changed from original implementation by Cong Ma, 2016
{

    // Reverse the signs of "DistMatrix":
	// This is necessary as the original code by Markus Buehren assumes that the input matrix is a cost matrix,
	// but in this work we assume input matrix is a profit matrix.
	for (unsigned int i = 0; i < DistMatrix.size(); i++)  // Changed from original implementation by Cong Ma, 2016
		for (unsigned int j = 0; j < DistMatrix[i].size(); j++) // Changed from original implementation by Cong Ma, 2016
			DistMatrix[i][j] = -DistMatrix[i][j]; // Changed from original implementation by Cong Ma, 2016
	...
	delete[] distMatrixIn;
	delete[] assignment;
	return -cost;  // Changed from original implementation by Cong Ma, 2016
}
...
void HungarianAlgorithm::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	for (row = 0; row<nOfElements; row++)
	{
		...
		//if (value < 0) // Changed from original implementation by Cong Ma, 2016
		//	cerr << "All matrix elements have to be non-negative." << endl; // Changed from original implementation by Cong Ma, 2016
