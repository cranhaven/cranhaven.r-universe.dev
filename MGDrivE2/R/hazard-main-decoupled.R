...
#' @param type string indicating type of hazards, one of; "life", "SIS", "Imperial" or "SEIR"
...
#'
#' @export
spn_hazards_decoupled <- function(spn_P,spn_T,cube,params,type="SIS",
  ...
  infNames <- c("female_inf")

  # the infection hazard depends on the type of sampling, SIS or Imperial
  if(type == "Imperial") {
    infFuncs <- setNames(object = c(make_female_inf_epi_haz_decoupled_Imperial),
                      nm = infNames)
  } else {  
    infFuncs <- setNames(object = c(make_female_inf_epi_haz_decoupled_SIS),
                      nm = infNames)
  }
