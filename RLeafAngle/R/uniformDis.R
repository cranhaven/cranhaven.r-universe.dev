...
#' @description Compute the uniform distribution of leaf zenith angle.
#' @return The uniform distribution of leaf zenith angle.
...
uniformDis<-function()
{
  ...
  {
    fraction<-integrate(uniformLAD, lower=angz[i]*pi180, upper=angz[i+1]*pi180)
    sDis[i]<-fraction[[1]]
  }
  sDis
}