 plot.pencopula <- function(x,val=NULL,marg=TRUE,plot=TRUE,int=FALSE,main.txt=NULL,
                    ...
                    xlab=NULL,ylab=NULL,zlab=NULL,zlim=NULL,biv.margin=NULL,show.observ=FALSE,cond.cop=FALSE,cond.par,margin.normal=FALSE,...) {
  env <- list()
  p <- get("p",x)
  q <- get("q",x)
  d <- get("d",x)
  ...
  if(is.null(val))
    {
      if(p!=2 & (!cond.true & is.null(biv.margin)&!cond.cop)) stop("geht nicht!")
      else {
        if(p==2) {
        ...
        if(cond.true & is.null(biv.margin)&!cond.cop) {
        ...
        if(cond.true|cond.cop) {
          x.grid <- seq(0,1,length=51)
          if(cond.true) grid <- expand.grid(y1=x.grid, y2=x.grid)
          if(margin.normal) {
             x.grid.unif <- seq(-5,5,length=51)
             x.grid <- pnorm(x.grid.unif)
             grid <- expand.grid(y1=x.grid, y2=x.grid) 
          ...
          if(cond.cop) {
             grid <- expand.grid(y1=x.grid, y2=x.grid,y3=rep(cond.par,51))
             #p<-3
             if(margin.normal) {
               x.grid.unif <- seq(-5,5,length=51)
               x.grid <- pnorm(x.grid.unif)
               grid <- expand.grid(y1=x.grid, y2=x.grid,y3=rep(pnorm(cond.par),51))
             } 
          }
          #if(cond.true) grid <- expand.grid(y1=x.grid, y2=x.grid)
          ...
          if(margin.normal&cond.true) grid[["plot"]]<-grid[["plot"]]*dnorm(grid$y1)*dnorm(grid$y2)
          if(margin.normal&cond.cop) grid[["plot"]]<-grid[["plot"]]*dnorm(grid$y1)*dnorm(grid$y2)*dnorm(grid$y3)
          grid$plot[which(grid$plot<0)]<-0
        }
        if(is.null(biv.margin)&!cond.cop) {
          tilde.Psi.d <-  array(NA, dim=c(dim(grid)[1],get("ddb",x),p))
          for (j in 1:p)
            {
          ...
          if(margin.normal) grid[["plot"]] <- (tilde.PSI.d.D%*%ck)*dnorm(grid$y1)*dnorm(grid$y2)
          else grid[["plot"]]<- tilde.PSI.d.D%*%ck
          grid$plot[which(grid$plot<0)]<-0
        }
        if(!is.null(biv.margin)&!cond.cop) {
          ...
          for(j in 1:p) {
            name <- c(paste("y",j,sep=""))
            env[[noquote(name)]] <- seq(0,1,length=51)
            if(margin.normal) {
              x.grid.unif <- seq(-5,5,length=51)
              x.grid <- pnorm(x.grid.unif)
              env[[noquote(name)]] <- x.grid
            }
          ...
          grid[["plot"]] <- tilde.PSI.d.D%*%ck
          if(margin.normal) grid[["plot"]]<-grid[["plot"]]*dnorm(grid$y1)*dnorm(grid$y2)
          show.ob <- biv.margin
          grid$plot[which(grid$plot<0)]<-0
        }
        ...
        if(is.null(zlim)) zlim <- c(0,max(grid$plot))
        ...
        if(contour&!show.observ) obj1 <- contourplot(values, data=grid,outer=TRUE,sub=sub.txt,zlab=list(label=z.txt,cex=cex.axes),xlab=list(label=xlab,cex=cex.axes),
                                 ...
                                 zlim=c(0,max(grid$plot)),main=main.txt,shade=TRUE,cuts=cuts,subscripts=TRUE)

        if(contour&show.observ) {
          yy <- NA
          rm(yy)
          grid2<-data.frame(xx=get("Y",x)[,biv.margin[1]],yy=get("Y",x)[,biv.margin[2]])
          obj1 <- contourplot(values, data=grid,outer=TRUE,sub=sub.txt,zlab=list(label=z.txt,cex=cex.axes),
                              ...
                              zlim=c(0,max(grid$plot)),main=main.txt,shade=TRUE,cuts=cuts,subscripts=TRUE)+layer(panel.points(x=xx,y=yy,subscripts=TRUE,under=TRUE,pch=1),data=grid2)
        }
        if(marg) {
          T.marg <- get("T.marg",x)
