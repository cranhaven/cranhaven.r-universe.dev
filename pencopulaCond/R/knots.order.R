knots.order <- function(penden.env) {
  ...
  for(i in 0:(get("d",penden.env)-1)) {
    ...
    if((get("q",penden.env)==2)&(i<(d-1))) x.order <- c(x.order,ungerade[help.seq]/((2)^(i+1))) #(2+ind)^
  }
  if(get("q",penden.env)==2) {
    val<-c(x.order*2^get("d",penden.env)+1,seq(2,2^get("d",penden.env),by=2))
    val[2]<-val[2]+1
    val<-c(val,val[length(val)]+1)
    return(val)
  }
