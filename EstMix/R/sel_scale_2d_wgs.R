...
#' @keywords internal
#' @export
sel_scale_2d_wgs = function(cnMax=6,pscnMax=6,ngrid = 100,nslaves = 50,temp){
  ##load(fn)
  ...
  sim_func <- function(ss,ngrid=100,pscnMax=6){
    ...
    for(i in which(!is.na(baf))){
      L_tot_2d[i,,]=calcll_baf(lrr[i],nrc[i],baf[i],n_baf[i],lprior_f_2d*0,rlprior_f_2d*0,scale,6,6)[[1]]
    }
    L_sum_2d = apply(L_tot_2d[which(!is.na(baf)),,],c(2,3),sum)
    out = min(L_sum_2d)
    return(out)
  }
