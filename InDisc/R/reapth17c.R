    ...
    if (display==TRUE){
      ...
      if (compT > 3600){
        if (days >= 1){ #Very very rare, but just to be sure
          ...
          cat("Computing Appropiateness indices. Time remaining: ", hours,"hours, ",minutes, "minutes and ",seconds, "seconds \r")
          flush.console()
        }
      }
      else{
        if (compT >= 60){
          cat("Computing Appropiateness indices. Time remaining: ", minutes, "minutes and ",seconds,"seconds \r")
          flush.console()
        }
        if (compT < 60) {
          cat("Computing Appropiateness indices. Time remaining ",seconds,"seconds                                                                  \r")
          flush.console()
        }
      }
    }
