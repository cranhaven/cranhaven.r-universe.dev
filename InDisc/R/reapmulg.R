reapmulg <- function(DIF, RES, alpha, vres, nodos1, nodos2, nfac, display){
  ...
  for (i in 1:n){
    ...
    if (nfac==2){
      OUT <- eapmulg2(DIF, RES[i,], alpha, vres, nodos1, nodos2)
      tmp1<-OUT$th
      tmp2<-OUT$vi
      tmp3<-OUT$st
      tmp4<-OUT$sv
    ...
    if (nfac==3){
      OUT <- eapmulg3(DIF, RES[i,], alpha, vres, nodos1, nodos2)
      tmp1<-OUT$th
      tmp2<-OUT$vi
      tmp3<-OUT$st
      tmp4<-OUT$sv
    ...
    if (nfac==4){
      OUT <- eapmulg4(DIF, RES[i,], alpha, vres, nodos1, nodos2)
      tmp1<-OUT$th
      tmp2<-OUT$vi
      tmp3<-OUT$st
      tmp4<-OUT$sv
