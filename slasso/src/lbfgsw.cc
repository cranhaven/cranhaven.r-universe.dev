...
)
{
  ...
  if (0.!=param.lambda) {
    /* Compute the L1 norm of the variable and add it to the object value. */
      xnorm = owlqn_x1norm(x, param.orthantwise_start, param.orthantwise_end,c);
      fx += xnorm * param.lambda;
      owlqn_pseudo_gradient(
        pg, x, g, n,
        param.lambda, param.orthantwise_start, param.orthantwise_end,c
      );
  }
  ...
  */
  ...
  for (;;) {
    ...
    /* Search for an optimal step. */
      if (0.==param.lambda) {
        ls = linesearch(n, x, &fx, g, d, &step, xp, gp, w, &cd, &param,c);
      } else {
        ls = linesearch(n, x, &fx, g, d, &step, xp, pg, w, &cd, &param,c);
        owlqn_pseudo_gradient(
          pg, x, g, n,
          param.lambda, param.orthantwise_start, param.orthantwise_end,c
        );
    ...
    *f = cd->proc_evaluate(cd->instance, x, g, cd->n, *stp);
    ...
    norm = owlqn_x1norm(x, param->orthantwise_start, param->orthantwise_end,c);
    *f += norm * param->lambda;
    ...
    *t = newt;
    ...
    lbfgsfloatval_t *c = lbfgs_malloc(N);
    ...
    if (invisible == 1){
    ret = lbfgs(N, x, &fx, evaluate, NULL, fPointer, &param,c);
    }
    else{
    ret = lbfgs(N, x, &fx, evaluate, progress, fPointer, &param,c);
    }
