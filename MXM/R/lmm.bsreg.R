lmm.bsreg <- function(target, dataset, id, threshold = 0.05, wei = NULL) {
  ...
  if ( p > n ) {
    ...
    stat <- numeric(p)
    if (p == 1) {
      mod <- lme4::lmer( target ~ 1 + (1 | id), REML = FALSE, weights = wei )
      stat <- 2 * ( likini - logLik(mod) )	
	  } else {
      for (j in 1:p) {
        mod <- lme4::lmer( target ~ dataset[, -j, drop = FALSE] + (1 | id), REML = FALSE, weights = wei )
    ...
    if ( mat[sel, 2] < threshold ) {
      ...
      if ( info[1, 2] > threshold & dim(mat)[1] > 0) {
        ...
        while ( info[i, 2] > threshold  &  dim(dat)[2] > 0 )  {   
          ...
          ini <- lme4::lmer( target ~ dat + (1 | id), REML = FALSE, weights = wei )
          likini <- logLik(ini) 
          i <- i + 1        
          k <- p - i + 1
          ...
          if ( k == 1 ) {
            ...
            for (j in 1:k) {
              mod <- lme4::lmer( target ~ dat[, -j, drop = FALSE] + (1 | id), REML = FALSE, weights = wei )
              stat[j] <- 2 * ( likini - logLik(mod) )
            }
