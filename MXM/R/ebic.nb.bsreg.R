...
ebic.nb.bsreg <- function(target, dataset, wei = NULL, gam = NULL) {
  ...
  if ( p > n ) {
    ...
    ini <- MASS::glm.nb( target ~.,  data = dataset, weights = wei )
    ...
    if ( M == 0 ) {
      mod <- MASS::glm.nb( target ~ 1, weights = wei)
      bic <- BIC(mod)      
      if (bic0 - bic < 0 ) {
      ...
      ###########   
    for (j in 1:p) {
      mod <- MASS::glm.nb( target ~.,  data = dataset[, -j, drop = FALSE], weights = wei)
    ...
    if ( bic0 - mat[sel, 2] < 0  ) {
      ...
      if ( tool[2] != 0 ) {
        ...
        while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
          ...
          ini <- MASS::glm.nb( target ~., data = dat, weights = wei )
          M <- dim(dat)[2]
          ...
          if ( M == 1 ) {
            ...
            M <- dim(dat)[2] - 1
            for ( j in 1:(M + 1) ) {
              mod <- MASS::glm.nb( target ~.,  data = dat[, -j, drop = FALSE], weights = wei )
              bic[j] <-  BIC(mod) + con * lchoose(p, M)
            }
            mat[, 2] <- bic
            sel <- which.min( mat[, 2] )
