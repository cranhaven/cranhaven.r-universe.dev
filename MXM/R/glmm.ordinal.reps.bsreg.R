glmm.ordinal.reps.bsreg <- function(target, dataset, id, reps, threshold = 0.05, wei = NULL, test = "testIndGLMMReg") {
    ...
    if ( p > n ) {
      ...
      tic <- proc.time()
      ###################
      ini <- ordinal::clmm( target ~ dataset + reps + (1 | id), weights = wei )
      likini <- logLik(ini) 
      stat <- numeric(p)
      if ( p == 1 )  {
        mod <- ordinal::clmm( target ~ 1 + reps + (1 | id), weights = wei )
        stat <- 2 * ( likini - logLik(mod) )	
      } else {
        for (j in 1:p) {
          mod <- ordinal::clmm( target ~ dataset[, -j, drop = FALSE] + reps + (1 | id), weights = wei )
      ...
      if ( mat[sel, 2] < threshold ) {
        ...
        if ( info[1, 2] > threshold & dim(mat)[1] > 0) {
          ...
          while ( info[i, 2] > threshold  &  dim(dat)[2] > 0 )  {   
            ...
            ini <- ordinal::clmm( target ~ dat + reps + (1 | id), weights = wei )
            likini <- logLik(ini) 
            i <- i + 1        
            k <- p - i + 1
            
            if ( k == 1 ) {
              mod <- ordinal::clmm(target ~ 1 + reps + (1 | id), REML = FALSE, weights = wei)
              stat <- 2 * ( likini - logLik(mod) )
              ...
              for (j in 1:k) {
                mod <- ordinal::clmm( target ~ dat[, -j, drop = FALSE] + reps + (1 | id), weights = wei )
                stat[j] <- 2 * ( likini - logLik(mod) )
              }
