testIndGLMMNB = function(target, reps = NULL, group, dataset, xIndex, csIndex, wei =  NULL, univariateModels = NULL,
 hash = FALSE, stat_hash = NULL, pvalue_hash = NULL, slopes = FALSE) {
  ...
  if (length(cs) == 0)  {
   ...
   if ( is.null(reps) ) {
     fit1 <- lme4::glmer.nb( target ~ (1|group), weights = wei, nAGQ = 0 ) 
     fit2 <- lme4::glmer.nb( target ~ (1|group) + dataset[, xIndex], weights = wei, nAGQ = 0 ) 
   } else {
      reps <- reps 
      if ( slopes ) {
        fit1 <- lme4::glmer.nb( target ~ reps + (reps|group), weights = wei, nAGQ = 0 ) 
        fit2 <- lme4::glmer.nb( target ~ reps + (reps|group) + x, weights = wei, nAGQ = 0 ) 
      } else {
        reps <- reps 
        fit1 <- lme4::glmer.nb( target ~ reps + (1|group), weights = wei, nAGQ = 0 )  
        fit2 <- lme4::glmer.nb( target ~ reps + (1|group) + x, weights = wei, nAGQ = 0 )  
      }
   }
   
  } else {
    if ( is.null(reps) ) {
      fit1 <- lme4::glmer.nb( target ~ (1|group) + cs, weights = wei, nAGQ = 0 )    
      fit2 <- lme4::glmer.nb( target ~ (1|group) + cs + x, weights = wei, nAGQ = 0 )    
    } else {
      reps <- reps 
      if (slopes ) {
        fit1 <- lme4::glmer.nb( target ~ reps + (reps|group) + cs, weights = wei, nAGQ = 0 )
        fit2 <- lme4::glmer.nb( target ~ reps + (reps|group) + cs + x, weights = wei, nAGQ = 0)
      } else {
        fit1 <- lme4::glmer.nb( target ~ reps + (1|group) + cs, weights = wei, nAGQ = 0 ) 
        fit2 <- lme4::glmer.nb( target ~ reps + (1|group) + cs + x, weights = wei, nAGQ = 0 ) 
      }
    }  
  }
