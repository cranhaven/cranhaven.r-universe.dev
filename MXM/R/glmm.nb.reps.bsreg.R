glmm.nb.reps.bsreg <- function(target, dataset, id, reps, threshold = 0.05, wei = NULL) {
    ...
    if ( p > n ) {
      ...
      ###################
      ###################
      ini <- lme4::glmer.nb( target ~ dataset + reps + (1 | id), weights = wei, nAGQ = 0 )
      likini <- logLik(ini) 
      stat <- numeric(p)
      if ( p == 1 )  {
        mod <- lme4::glmer.nb( target ~ 1 + reps + (1 | id), weights = wei, nAGQ = 0 )
        stat <- 2 * ( likini - logLik(mod) )	
      } else {
        for (j in 1:p) {
          mod <- lme4::glmer.nb( target ~ dataset[, -j, drop = FALSE] + reps + (1 | id), weights = wei, nAGQ = 0 )
      ...
      if ( mat[sel, 2] < threshold ) {
        ...
        if ( info[1, 2] > threshold & dim(mat)[1] > 0) {
          ...
          while ( info[i, 2] > threshold  &  dim(dat)[2] > 0 )  {   
            ...
            ini <- lme4::glmer.nb( target ~ dat + reps + (1 | id), weights = wei, nAGQ = 0 )
            likini <- logLik(ini) 
            i <- i + 1        
            k <- p - i + 1
            
            if ( k == 1 ) {
              mod <- lme4::glmer.nb( target ~ 1 + reps + (1 | id), REML = FALSE, weights = wei, nAGQ = 0 )
              stat <- 2 * ( likini - logLik(mod) )
              ...
              for (j in 1:k) {
                mod <- lme4::glmer.nb( target ~ dat[, -j, drop = FALSE] + reps + (1 | id), weights = wei, nAGQ = 0 )
                stat[j] <- 2 * ( likini - logLik(mod) )
              }
