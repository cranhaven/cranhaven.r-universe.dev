fbed.glmm.nb <- function(y, x, id, prior = NULL, univ = NULL, alpha = 0.05, wei = NULL, K = 0) { 
  ...
  sig <- log(alpha)
  if ( is.null(prior) ) {
    lik1 <- logLik( lme4::glmer.nb( y ~ 1 + (1|id), weights = wei, nAGQ = 0 ) )
  } else  lik1 <- logLik( lme4::glmer.nb( y ~ prior + (1|id), weights = wei, nAGQ = 0 ) )
  lik2 <- numeric(p)
  sela <- NULL
  card <- 0
  sa <- NULL
  ...
  if ( is.null(univ) ) {
    for ( i in ind ) {
      fit2 <- lme4::glmer.nb( y ~ x[, c(i, priorindex)] + (1|id), weights = wei, nAGQ = 0 )
      lik2[i] <- logLik( fit2 )
    }
    n.tests <- p
    stat <- 2 * (lik2 - lik1)
  ...
  if ( length(s) > 0 ) {
    ...
    while ( sum(s>0) > 0 ) {
      for ( i in ind[s] )  {
        fit2 <- lme4::glmer.nb( y ~ x[, c(priorindex, sela, i)] + (1|id), weights = wei, nAGQ = 0 )
        lik2[i] <- logLik( fit2 )
      }
