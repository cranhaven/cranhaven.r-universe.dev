fbed.geeglm <- function(y, x, id, prior = NULL, univ = NULL, alpha = 0.05, wei = NULL, K = 0, test = "testIndGEELogistic", correl = "exchangeable", se = "jack") { 
  ...
  if ( is.null(univ) ) {
    if ( is.null(prior) ) {
      for ( i in ind ) {
        fit2 <- try( geepack::geeglm( y ~ x[, i], family = oiko, id = id, weights = wei, corstr = correl, std.err = se ), silent = TRUE )
      ...
      for ( i in ind ) {
        fit2 <- try( geepack::geeglm( y ~., data = x[, c(priorindex, i)], family = oiko, id = id, weights = wei, corstr = correl, std.err = se ), silent = TRUE)
        if ( identical( class(fit2), "try-error" ) ) {
          stat[i] <- 0
        } else {  
          mod <- anova(fit2)
  ...
  if ( length(s) > 0 ) {
    ...
    while ( sum(s>0) > 0 ) {
      for ( i in ind[s] )  {
        fit2 <- try( geepack::geeglm( y ~., data = x[, c(priorindex, sela, i)], family = oiko, id = id, weights = wei, corstr = correl, std.err = se ), silent = TRUE)
        if ( identical( class(fit2), "try-error" ) ) {
          stat[i] <- 0
        } else {  
          mod <- anova(fit2)
