cv_method_single_MC_corr <-
function(mu0, p, m, n, alpha_list, p1, ss, ntest, pcorr, chol.rho, sampling.p )
{
	...
	testdata=gen_data_corr(mu0, p, m, ntest, pcorr, chol.rho)
	...
	{
		cl_cutoff=1/2*log((1-p1)/p1)*dot(weight, weight)/dot(zscore/sqrt(nrow(data$x)), weight)
