#' @title Specify a smooth, increasing and convex shape-restriction in a \pkg{bcgam} formula
#'
...
#' @export
#'
#' @details \code{sm.incr.conv} returns the vector \code{x} and assigns five attributes to it: name, shape (5 for  
...
sm.incr.conv <- function (x, numknots = 0, knots = 0, space = "Q") 
{
    cl <- match.call()
    pars <- match.call()[-1]
