#' @title Specify a smooth, decreasing and convex shape-restriction in a \pkg{bcgam} formula
#'
...
#' @export
#'
#' @details \code{sm.decr.conv} returns the vector \code{x} and assigns five attributes to it: name, shape (7 for  
...
sm.decr.conv <- function (x, numknots = 0, knots = 0, space = "Q") 
{
    cl <- match.call()
    pars <- match.call()[-1]
