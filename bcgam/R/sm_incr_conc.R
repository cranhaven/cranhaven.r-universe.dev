#' @title Specify a smooth, increasing and concave shape-restriction in a \pkg{bcgam} formula
#'
...
#' @export
#'
#' @details \code{sm.incr.conc} returns the vector \code{x} and assigns five attributes to it: name, shape (1 for  
...
sm.incr.conc <- function (x, numknots = 0, knots = 0, space = "Q") 
{
    cl <- match.call()
    pars <- match.call()[-1]
