#' @title Specify a smooth and increasing shape-restriction in a \pkg{bcgam} formula
#'
...
#' @export
#'
#' @details \code{sm.incr} returns the vector \code{x} and assigns five attributes to it: name, shape (1 for  
...
sm.incr <- function (x, numknots = 0, knots = 0, space = "Q") 
{
    cl <- match.call()
    pars <- match.call()[-1]
