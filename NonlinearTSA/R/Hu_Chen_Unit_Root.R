...
#' @importFrom stats AIC BIC lm embed residuals
#' @examples
#'
#' x <- rnorm(1000)
...
#'
#'
Hu_Chen_Unit_Root<-function(x,case,lags,lsm){
  if(lags == 0){
  ...
  for(i in 1:lags){
    ...
    model<-lm(z.diff~z.lag.1+z.lag.2+z.lag.3+0+z.diff.lag )

    AICs[i+1] = AIC(model)
    BICs[i+1] = BIC(model)
    tstats[i+1] = summary(model)$coefficients[(i+3),4]
  }
  ...
  kendizero = x
  z.lag.zero.3=kendizero[2:n]
  model0<-lm(z.diffzero~z.lag.zero.1+z.lag.zero.2+z.lag.zero.3+0)
  AICs[1] = AIC(model0)
  BICs[1] = BIC(model0)
  tstats[1] = 0.0000
  ...
  kendi=x
  z.lag.3=kendi[(uygun_lag+1):n]
  k=uygun_lag+1
  if(uygun_lag == 0){
    son <- lm(z.diff~z.lag.1+z.lag.2+z.lag.3+0)
    ay <- linearHypothesis(son, c("z.lag.1=0","z.lag.2=0","z.lag.3=0"), test="Chisq")
  } else {
    z.diff.lag = embed(z, uygun_lag+1)[, 2:k]
    son <- lm(z.diff~z.lag.1+z.lag.2+z.lag.3+0+z.diff.lag)
    ay <- linearHypothesis(son, c("z.lag.1=0","z.lag.2=0","z.lag.3=0"), test="Chisq")
  }
