...
dKINBII=
  function (x, mu = 1, sigma = 1, nu = 0.3, kinf=0 ,log = FALSE)
  {
    ...
    fy <- dNBII(x, mu = mu, sigma = sigma, log = T)
...
qKINBII=
            ...
            log.p = FALSE)
  {
    ...
    nu <- rep(nu, length = ly)
    inf<- rep(kinf, length = ly)
    pnew <-ifelse(p>pKINBII(max(inf-1,0), mu=mu, sigma=sigma, nu=nu, kinf=inf),
                  (p - nu)/(1 - nu)-(1e-07), p /(1 - nu)-(1e-07))
    pnew <-ifelse( inf==0,(p - nu)/(1 - nu)-(1e-07), pnew)
    pnew <- ifelse((pnew > 0), pnew, 0)
    q <- qNBII(pnew, mu = mu, sigma = sigma)
    q
  }
#___________________________________________________________________________________________________________________
...
rKINBII=
  function (n, mu = 1, sigma = 1, nu = 0.3, kinf=0)
  {
    ...
    for(i in 1:n){
      if(p[i]<nu){r[i]=kinf}
      else{r[i] = rNBII(1, mu = mu, sigma = sigma)}
    }
    r
  }
...
KINBII=
  ...
  {
    mstats <- checklink("mu.link", "kINBII", substitute(mu.link),
                        c("inverse", "log", "identity"))
    dstats <- checklink("sigma.link", "kINBII", substitute(sigma.link),
                        c("inverse", "log", "identity"))
    vstats <- checklink("nu.link", "kINBII", substitute(nu.link),
                        c("logit", "probit", "cloglog", "log", "own"))
    structure(list(family = c(paste("inf",kinf,"NBII", sep = ""),paste(kinf,"-inflated negative binomial type II", sep = "") ),
                   ...
                   dldm = function(y, mu, sigma, nu) {
                     dldm0 <- (1 - nu) * ((nu + (1 - nu) * dNBII(kinf, mu,
                                                                 sigma))^(-1)) * dNBII(kinf, mu, sigma) * NBII()$dldm(kinf,
                                                                                                                      mu, sigma)
                     dldm <- ifelse(y == kinf, dldm0, NBII()$dldm(y, mu, sigma))
                     dldm
                     ...
                     dldd0 <- (1 - nu) * ((nu + (1 - nu) * dNBII(kinf, mu,
                                                                 sigma))^(-1)) * dNBII(kinf, mu, sigma) * NBII()$dldd(kinf,
                                                                                                                      mu, sigma)
                     dldd <- ifelse(y == kinf, dldd0, NBII()$dldd(y, mu, sigma))
                     dldd
                     ...
                     dldv0 <- ((nu + (1 - nu) * dNBII(kinf, mu, sigma))^(-1)) *
                       (1 - dNBII(kinf, mu, sigma))
                     ...
                     dKINBII(y, mu = mu, sigma = sigma, nu = nu, kinf=kinf, log = TRUE),
                   rqres = expression(rqres(pfun = "pKINBII", type = "Discrete",
                                            ymin = 0, y = y, mu = mu, sigma = sigma, nu = nu)),
                   mu.initial = expression(mu <- (y + mean(y))/2), sigma.initial = expression(sigma <- rep(max(((var(y)/mean(y)) -
                  1), 0.1), length(y))), nu.initial = expression(nu <- rep(((sum(y ==
                  kinf)/length(y)) + 0.01)/2, length(y))), mu.valid = function(mu) all(mu >
                  0), sigma.valid = function(sigma) all(sigma > 0),
