...
dKIGPO=
  function (x, mu = 1, sigma = 1, nu = 0.3, kinf=0 ,log = FALSE)
  {
    ...
    fy <- dGPO(x, mu = mu, sigma = sigma, log = T)
...
qKIGPO=
            ...
            log.p = FALSE)
  {
    ...
    nu <- rep(nu, length = ly)
    inf<- rep(kinf, length = ly)
    pnew <-ifelse(p>pKIGPO(max(inf-1,0), mu=mu, sigma=sigma, nu=nu, kinf=inf),
                  (p - nu)/(1 - nu)-(1e-07), p /(1 - nu)-(1e-07))
    pnew <-ifelse( inf==0,(p - nu)/(1 - nu)-(1e-07), pnew)
    pnew <- ifelse((pnew > 0), pnew, 0)
    q <- qGPO(pnew, mu = mu, sigma = sigma)
    q
  }
#___________________________________________________________________________________________________________________
...
rKIGPO=
  function (n, mu = 1, sigma = 1, nu = 0.3, kinf=0)
  {
    ...
    for(i in 1:n){
      if(p[i]<nu){r[i]=kinf}
      else{r[i] = rGPO(1, mu = mu, sigma = sigma)}
    }
    r
  }
...
KIGPO=
  ...
  {
    mstats <- checklink("mu.link", "kIGPO", substitute(mu.link),
                        c("inverse", "log", "identity"))
    dstats <- checklink("sigma.link", "kIGPO", substitute(sigma.link),
                        c("inverse", "log", "identity"))
    vstats <- checklink("nu.link", "kIGPO", substitute(nu.link),
                        c("logit", "probit", "cloglog", "log", "own"))
    structure(list(family = c(paste("inf",kinf,"GPO", sep = ""),paste(kinf,"-inflated Generalised Poisson", sep = "") ),
                   ...
                   dldm = function(y, mu, sigma, nu) {
                     dldm0 <- (1 - nu) * ((nu + (1 - nu) * dGPO(kinf, mu,
                                                                sigma))^(-1)) * dGPO(kinf, mu, sigma) * GPO()$dldm(kinf,
                                                                                                                   mu, sigma)
                     dldm <- ifelse(y == kinf, dldm0, GPO()$dldm(y, mu, sigma))
                     dldm
                     ...
                     dldd0 <- (1 - nu) * ((nu + (1 - nu) * dGPO(kinf, mu,
                                                                sigma))^(-1)) * dGPO(kinf, mu, sigma) * GPO()$dldd(kinf,
                                                                                                                   mu, sigma)
                     dldd <- ifelse(y == kinf, dldd0, GPO()$dldd(y, mu, sigma))
                     dldd
                     ...
                     dldv0 <- ((nu + (1 - nu) * dGPO(kinf, mu, sigma))^(-1)) *
                       (1 - dGPO(kinf, mu, sigma))
                     ...
                     dKIGPO(y, mu = mu, sigma = sigma, nu = nu, kinf=kinf, log = TRUE),
                   rqres = expression(rqres(pfun = "pKIGPO", type = "Discrete", ymin = 0, y = y, mu = mu, sigma = sigma, nu = nu)),
                   mu.initial = expression(mu <- (y + mean(y))/2), sigma.initial = expression(sigma <- rep(0.1,
                  length(y))), nu.initial = expression(nu <- rep(((sum(y == kinf)/length(y)) + 0.01)/2, length(y))),
