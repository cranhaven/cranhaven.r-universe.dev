...
dKIWARING=
  function (x, mu = 1, sigma = 1, nu = 0.3, kinf=0 ,log = FALSE)
  {
    ...
    fy <- dWARING(x, mu = mu, sigma = sigma, log = T)
...
pKIWARING=
            ...
            log.p = FALSE)
  {
    ...
    nu <- rep(nu, length = ly)
    inf<- rep(kinf, length = ly)
    cdf <- pWARING(q, mu = mu, sigma = sigma)
...
qKIWARING=
            ...
            log.p = FALSE)
  {
    ...
    nu <- rep(nu, length = ly)
    inf<- rep(kinf, length = ly)
    pnew <-ifelse(p>pKIWARING(max(inf-1,0), mu=mu, sigma=sigma, nu=nu, kinf=inf),
                  (p - nu)/(1 - nu)-(1e-07), p /(1 - nu)-(1e-07))
    pnew <-ifelse( inf==0,(p - nu)/(1 - nu)-(1e-07), pnew)
    pnew <- ifelse((pnew > 0), pnew, 0)
    q <- qWARING(pnew, mu = mu, sigma = sigma)
    q
  }
#___________________________________________________________________________________________________________________
...
rKIWARING=
  function (n, mu = 1, sigma = 1, nu = 0.3, kinf=0)
  {
    ...
    for(i in 1:n){
      if(p[i]<nu){r[i]=kinf}
      else{r[i] = rWARING(1, mu = mu, sigma = sigma)}
    }
    r
  }
...
KIWARING=
  ...
  {
    mstats <- checklink("mu.link", "kIWARING", substitute(mu.link),
                        c("inverse", "log", "identity"))
    dstats <- checklink("sigma.link", "kIWARING", substitute(sigma.link),
                        c("inverse", "log", "identity"))
    vstats <- checklink("nu.link", "kIWARING", substitute(nu.link),
                        c("logit", "probit", "cloglog", "log", "own"))
    structure(list(family = c(paste("inf",kinf,"WARING", sep = ""),paste(kinf,"-inflated waring", sep = "") ),
                   ...
                   dldm = function(y, mu, sigma, nu) {
                     dldm0 <- (1 - nu) * ((nu + (1 - nu) * dWARING(kinf, mu,
                                                                   sigma))^(-1)) * dWARING(kinf, mu, sigma) * WARING()$dldm(kinf,
                                                                                                                            mu, sigma)
                     dldm <- ifelse(y == kinf, dldm0, WARING()$dldm(y, mu, sigma))
                     dldm
                     ...
                     dldd0 <- (1 - nu) * ((nu + (1 - nu) * dWARING(kinf, mu,
                                                                   sigma))^(-1)) * dWARING(kinf, mu, sigma) * WARING()$dldd(kinf,
                                                                                                                            mu, sigma)
                     dldd <- ifelse(y == kinf, dldd0, WARING()$dldd(y, mu, sigma))
                     dldd
                     ...
                     dldv0 <- ((nu + (1 - nu) * dWARING(kinf, mu, sigma))^(-1)) *
                       (1 - dWARING(kinf, mu, sigma))
                     ...
                     dKIWARING(y, mu = mu, sigma = sigma, nu = nu, kinf=kinf, log = TRUE),
                   rqres = expression(rqres(pfun = "pKIWARING", type = "Discrete",
                                            ymin = 0, y = y, mu = mu, sigma = sigma, nu = nu)),
                   mu.initial = expression(mu <- rep(mean(y), length(y))),
                   sigma.initial = expression(sigma <- rep(1, length(y))), nu.initial = expression(nu <- rep(((sum(y ==
