...
pKIGEOM=
  function (q, mu = 1, sigma = 0.1, kinf=0, lower.tail = TRUE, log.p = FALSE)
  {
    ...
    inf<- rep(kinf, length = ly)
    cdf <- rep(kinf, ly)
    cdf <- pGEOM(q, mu = mu, lower.tail = TRUE, log.p = FALSE)
...
qKIGEOM=
  function (p, mu = 1, sigma = 0.1, kinf=0, lower.tail = TRUE, log.p = FALSE)
  {
    ...
    mu <- rep(mu, length = ly)
    inf<- rep(kinf, length = ly)
    pnew <-ifelse(p>pKIGEOM(max(inf-1,0), mu=mu, sigma=sigma, kinf=inf),
...
KIGEOM=
  ...
  {
    mstats <- checklink("mu.link", "KIGEOM", substitute(mu.link),
                        c("1/mu^2", "log", "identity"))
    dstats <- checklink("sigma.link", "KIGEOM", substitute(sigma.link),
                        c("logit", "probit", "cloglog", "cauchit", "log", "own"))
    structure(list(family = c(paste("inf",kinf,"GEOM", sep = ""), paste(kinf,"-inflated Geometric", sep = "") ),
                   ...
                   sigma.dr = dstats$mu.eta, dldm = function(y, mu, sigma) {
                     dldm0 <- (1 - sigma) * ((sigma + (1 - sigma) * dGEOM(kinf, mu))
                                             ^(-1)) * dGEOM(kinf, mu) * GEOM()$dldm(kinf, mu)
                     dldm <- ifelse(y == kinf, dldm0, GEOM()$dldm(y, mu))
                     dldm
                     ...
                     dldd0 <- ((sigma + (1 - sigma) * dGEOM(kinf, mu))^(-1)) *
                       (1 - dGEOM(kinf, mu))
                     ...
                     d2ldmdd <- -dldm * dldd
                     d2ldmdd
                   }, G.dev.incr = function(y, mu, sigma, kinf, ...) -2 * dKIGEOM(y,
                    mu, sigma, kinf=kinf, log = TRUE), rqres = expression(rqres(pfun = "pKIGEOM",
                   type = "Discrete", ymin = 0, y = y, mu = mu, sigma = sigma, kinf=kinf)),
                   mu.initial = expression(mu <- rep(mean(y), length(y))),
                   sigma.initial = expression(sigma <- rep(0.1,length(y))), mu.valid = function(mu) all(mu > 0),
                   sigma.valid = function(sigma) all(sigma > 0 & sigma <1), y.valid = function(y) all(y >= 0)),
