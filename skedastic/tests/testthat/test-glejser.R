context("glejser works for two lm examples across all argument permutations")


theargs <- formals(glejser)
...
theargs <- list("sigmaest" = c("main", "auxiliary"), "auxdesign" = list(NA, carsauxmat,
...
test_that("linear regression works with all combinations of formals", {
  pvals <- vapply(1:nrow(allargs), function(i) do.call(what = glejser,
