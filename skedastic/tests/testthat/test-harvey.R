context("harvey works for two lm examples across all argument permutations")


theargs <- formals(harvey)
...
test_that("linear regression works with all combinations of formals", {
  pvals <- vapply(1:nrow(allargs), function(i) do.call(what = harvey,
