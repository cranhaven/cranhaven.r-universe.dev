...
system.time({rst = FLSSS::mFLSSSparIntegerized(
  maxCore = 7, len = subsetSize, mV = superset, mTarget = subsetSum,
  mME = subsetSumError, solutionNeed = 2, dl = ncol(superset),
  du = ncol(superset), tlimit = 30, useBiSrchInFB = FALSE, avgThreadLoad = 8L)})
...
rst = FLSSS::mFLSSSparIntegerized(
  maxCore = 7, len = subsetSize, mV = superset, mTarget = subsetSum,
  mME = subsetSumError, solutionNeed = 2, dl = 3L, du = 4L, tlimit = 30,
