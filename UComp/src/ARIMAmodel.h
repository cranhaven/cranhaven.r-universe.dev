...
ARIMAclass preProcess(vec y, mat u, vec orders, double cnst, uword s, int h,
    ...
    if (criterion.compare("bic") != 0 && criterion.compare("aic") != 0 && criterion.compare("aicc") != 0){
        Rprintf("%s", "ERROR: incorrect information criterion!!!\n");
        errorExit = true;
    }
    // NaNs in u
    if (u.n_rows > 0 && u.has_nonfinite()){
        Rprintf("%s", "ERROR: missing values not allowed in input variables!!!\n");
        errorExit = true;
    }
    // orders
    if (orders.n_elem == 0){
    ...
    if (sum(orders) == 0 && cnst == 0.0){
        Rprintf("%s", "ERROR: no model to estimate!!!\n");
        errorExit = true;
    }
    if (orders.n_elem < 6){
        adjustVector(orders, 6, 0.0);
    ...
    if (sum(maxOrders - orders < 0) > 0){
        Rprintf("%s", "ERROR: maxOrders should be always bigger than orders!!!\n");
        errorExit = true;
    }
    if (s < 2){
    ...
    if (ind.n_elem < maxObs){
        ...
        if (ind.n_elem < 8 || ind.n_elem < maxObs){
            if (ind.n_elem < 8){
                Rprintf("%s", "Error: Too few output observations!!!\n");
                errorExit = true;
            }
            tooFew = true;
            bootstrap = false;
    ...
    if (cnst != 0.0 && cnst != 1.0 && cnst != 9999.9){
        Rprintf("%s", "ERROR: cnst input should be \'true\', \'false\' or \'nell\'!!!\n");
        errorExit = true;
    }
    // Remove constant variables from inputs
    if (u.n_rows > 0){
