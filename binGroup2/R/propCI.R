...
"bgtAC" <- function(n, y, s, conf.level = 0.95, alternative = "two.sided") {
  alpha <- 1 - conf.level
  
  est.int <- (y + (qnorm(1 - alpha / 2)^2) / 2) / 
    (n + (qnorm(1 - alpha / 2))^2)
  est.int1s <- (y + (qnorm(1 - alpha)^2) / 2) / (n + (qnorm(1 - alpha))^2)
  
  if (alternative == "two.sided") {
    AC.se <- (qnorm(1 - alpha / 2)) * 
      sqrt((est.int * (1 - est.int)) / (n + (qnorm(1 - alpha / 2))^2))
    ...
    AC.se <- (qnorm(1 - alpha)) * 
      sqrt((est.int1s * (1 - est.int1s)) / (n + (qnorm(1 - alpha))^2))
...
"bgtBlaker" <- function(n, y, s, conf.level = 0.95, 
...
"bgtCP" <- function(n, y, s, conf.level = 0.95, 
                    alternative = "two.sided"){
  lower <- 0
  upper <- 1
  if (alternative == "two.sided") {
...
"bgtSOC" <- function(n, s, y, conf.level = 0.95, 
                     alternative = "two.sided"){
  esti <- y / n
  ...
  if (alternative == "less") {
    upper <- midpo + kappa * 
      sqrt(esti * (1 - esti) + (gamma1 * esti * (1 - esti) + 
                                  gamma2) / n) / sqrt(n)
    CI <- c(0 ,upper)
...
"bgtWald" <- function(n, y, s, conf.level = 0.95, 
                      alternative = "two.sided"){
  if (y > n) {
    stop("number of positive tests y can not be greater than number of tests n")}
  th <- y / n
  esti <- 1 - (1 - th)^(1 / s)
  ...
  if (alternative == "two.sided") {
    snquant <- qnorm(p = 1 - alpha / 2, mean = 0, sd = 1, 
                     lower.tail = TRUE)
    CI <- c(esti - snquant * sqrt(var.esti), 
            esti + snquant * sqrt(var.esti))
  }
  else{if (alternative == "less") {
...
"bgtWilson" <- function(n, y, s, conf.level = 0.95, 
                        alternative = "two.sided") { 
  alpha <- 1 - conf.level 
  th <- y / n
  est.int <- (y + (qnorm(1 - alpha / 2)^2) / 2) / 
    (n + (qnorm(1 - alpha / 2))^2)
  est.int1s <- (y + (qnorm(1 - alpha)^2) / 2) / 
    (n + (qnorm(1 - alpha))^2)
  
  if (alternative == "two.sided") {
    w.se <- ((qnorm(1 - alpha / 2)) * 
               sqrt(n * th * (1 - th) + (qnorm(1 - alpha / 2)^2) / 4)) / 
      (n + qnorm(1 - alpha / 2)^2)
    ...
    w.se <- ((qnorm(1 - alpha)) * 
               sqrt(n * th * (1 - th) + (qnorm(1 - alpha)^2) / 4)) / 
      (n + qnorm(1 - alpha)^2)
...
"bgtCI" <- function(n, s, y, conf.level = 0.95, 
  ...
  switch(method,
         ...
         "Blaker" = {
           if (alternative == "less" || alternative == "greater") {
             ...
             conf.int <- c(NA, NA)}
           if (alternative == "two.sided") {
             conf.int <- bgtBlaker(n = n, s = s, y = y, 
                                   conf.level = conf.level)}
         },
         
         "score" = {conf.int <- bgtWilson(n = n, s = s, y = y, 
                                          conf.level = conf.level, 
                                          alternative = alternative)},
         
         "AC" = {conf.int <- bgtAC(n = n, s = s, y = y, 
                                   conf.level = conf.level, 
                                   alternative = alternative)},
         
         "Wald" = {conf.int <- bgtWald(n = n, s = s, y = y, 
                                       conf.level = conf.level, 
                                       alternative = alternative)},
         
         "soc" = {conf.int <- bgtSOC(n = n, s = s, y = y, 
                                     conf.level = conf.level, 
                                     alternative = alternative)}
  )
...
"bgtvs" <- function(n, s, y, conf.level = 0.95, 
  ...
  itlowerlimit <- function(pstart, outlower, alpha, n, s, 
    ...
    for (i in 2:maxiter) {
      crit[i] <- alpha - sum(apply(X = outlower, MARGIN = 1, 
                                   FUN = likHepI, n = n, s = s, p = p))
      if (sign(crit[i - 1]) != sign(crit[i])) {
        dir <- dir * (-1); step <- step / 2
      }
  ...
  if (alternative == "two.sided") {
    upper <- itupperlimit(pstart = point.esti, outupper = outupper, 
                          alpha = alpha / 2, n = n, s = s, maxiter = 100)
    lower <- itlowerlimit(pstart = point.esti, outlower = outlower, 
                          alpha = alpha / 2, n = n, s = s, maxiter = 100)
  }
  
  if (alternative == "less") {
    upper <- itupperlimit(pstart = point.esti, outupper = outupper, 
                          alpha = alpha, n = n, s = s, maxiter = 100)
    lower <- 0
  }
