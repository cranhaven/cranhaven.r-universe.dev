...
NI.D3.OTC2 <- function(p.vec, Se, Sp, ordering, group.sz, 
  ...
  for (I in set.of.I) {
    ...
    for (c in 1:ncol(possible.groups)) {
      ...
      save.it[counter,] <- c(I, save.info$Expt, save.info$Expt / I, 
  ...
  opt.ET <- list("OTC" = list("Stage1" = result.ET[1], 
                 ...
                 "p.mat" = joint.p,  "ET" = result.ET[2], 
                 "value" = result.ET[3], "Accuracy" = accuracy.mat)
  ...
  list("algorithm" = "Non-informative three-stage hierarchical testing",
       ...
       "opt.ET" = opt.ET, "Configs" = configs, "Top.Configs" = top.configs)
}
