...
NI.A2M.OTC2 <- function(p.vec, Se, Sp, group.sz, updateProgress = NULL, 
  ...
  for (I in set.of.I) {
    ...
    results.ATM[count,] <- c(I, N, results$ET_ATM * N,
                             ...
                             results$PSP1_ATM, results$PSP2_ATM, 
                             results$PPV1_ATM, results$PPV2_ATM, 
                             results$NPV1_ATM, results$NPV2_ATM)
  ...
  if (length(set.of.I) == 1) {
    ...
    colnames(configs.ATM) <- c("I", "N", "ET", "value", 
                               "PSe1", "PSe2", "PSp1", "PSp2", 
                               "PPPV1", "PPPV2", "PNPV1", "PNPV2")
    configs.ATM <- convert.config(algorithm = "A2M", results = configs.ATM, 
                                  diseases = 2)
  }
  ...
  joint.p <- matrix(data = rep(p.vec, each = opt.ATM[2]), 
                    nrow = 4, ncol = opt.ATM[2], 
  ...
  list("algorithm" = "Non-informative array testing with master pooling",
       ...
       "opt.ET" = list("OTC" = list("Array.dim" = opt.ATM[1], 
                                    "Array.sz" = opt.ATM[2]),
                       "p.mat" = joint.p, "ET" = opt.ATM[3], 
                       "value" = opt.ATM[4],
                       "Accuracy" = acc.ATM), 
       "Configs" = configs.ATM)
