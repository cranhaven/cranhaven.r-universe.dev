...
Inf.Dorf.OTC2 <- function(alpha = NULL, probabilities = NULL, Se, Sp, 
  ...
  for (N in set.of.blocks) {
    ...
    for (c in 1:ncol(possible.groups)) {
      ...
      save.it[counter,] <- c(joint.p, 
                             rep(NA, max(0, 4 * max(set.of.blocks) - length(joint.p))),
                             N, save.info$Expt, save.info$Expt / N, 
                             acc.dis1, acc.dis2, gp.sizes, 
                             rep(0, max(set.of.blocks) - length(gp.sizes)))
      counter <- counter + 1
    }
  ...
  accuracy.mat <- matrix(data = result.ET[4 * max(set.of.blocks) + 4:11], 
  ...
  opt.ET <- list("OTC" = list("Block.sz" = result.ET[4 * max(set.of.blocks) + 1], 
                 ...
                 "p.mat" = joint.p, 
                 "ET" = result.ET[4 * max(set.of.blocks) + 2], 
                 "value" = result.ET[4 * max(set.of.blocks) + 3], 
