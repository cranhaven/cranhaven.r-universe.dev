...
SEXP GillespieDirectGB(SEXP pre, SEXP post, SEXP h, SEXP M, SEXP T, SEXP delta,
		       SEXP runs, SEXP place, SEXP transition, SEXP rho)
{
  ...
  for (k=0; k<1000000000; k++)
    exp_rand();
  c1 = clock();
  Rprintf ("exp_rand: Elapsed CPU time: %f\n", (float) (c1 - c0)/CLOCKS_PER_SEC);

  c0 = clock();
  for (k=0; k<1000000000; k++)
    log(unif_rand());
  c1 = clock();
  Rprintf ("log(unif_rand): Elapsed CPU time: %f\n", (float) (c1 - c0)/CLOCKS_PER_SEC);
  ...
  */
  ...
  pnodeRoot->parent = 0;
  node *pnodeTransition = ppnodeLevel[iLevels-1];
  ...
  for (iLevel = 0; iLevel < iLevels; iLevel++) {
	...
	ppnodeLevel[iLevel][iNode].iTransition = iNode;
  ...
  for (iRun = 0; iRun < iRuns; iRun++) {
	...
	for(iTransitionPtr = 0; iTransitionPtr < piHazardsToModxRowTot[iLastTransition]; iTransitionPtr++) {
	  ...
	  pnode = &pnodeTransition[iTransition];
	  double dDeltaHazard = dNewHazard - pnode->dPartialAcumHazard;
	  do {
	...
	dTime += exp_rand() / pnode->dPartialAcumHazard;
	iUsedRandomNumbers++;
	...
	/*
	if (iTotTransitions == 55957790)
	  Rprintf("\n%d: iLastTransition = %d\n", iTotTransitions, iLastTransition);
	*/
	do {
	  // Find out which transition happened
	  double dRnd = unif_rand() * pnode->dPartialAcumHazard;
	  iUsedRandomNumbers++;
	  do {
	  ...
	  // Next check is needed because
	  // once in a while it is generated a number that goes past
	  // the last transition due to accumulated truncation errors.
	  // Discard this random number and try again.
	} while ((iLastTransition = pnode->iTransition) >= iTransitions);
