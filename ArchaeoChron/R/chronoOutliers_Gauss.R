#' Bayesian chronologies of Gaussian dates using the outlier modelling of Oxcal.
#'
...
chronoOutliers_Gauss <- function(M, s, refYear=NULL, outliersIndivVariance, outliersBernouilliProba, 
  ...
  jags <- jags.model(file = model.file, data = list('Nbobs' = length(M),'M' = M,'s'=s, 'sigma.delta' = outliersIndivVariance,'p'= outliersBernouilliProba,
                                                    'ta'=studyPeriodMin,'tb'=studyPeriodMax),n.chains = numberChains, n.adapt = numberAdapt)
  print('Update period')
