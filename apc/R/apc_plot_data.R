...
apc.plot.data.sums	<- function(apc.data.list,data.type="a",average=FALSE,keep.incomplete=TRUE,apc.index=NULL,type="o",log="",main.outer=NULL,main.sub=NULL,las=1,scale.response=1,scale.dose=1,scale.rate=1)
#	BN 13 Dec 2023: added mixed frequency
#	BN 15 Aug 2018: added possibility for sums/averages over incomplete sequences.
...
#			las				optional plot parameter. Numeric. "1" horizontal.
#			scale.response	optional. Numeric.  Multiplier for response
#			scale.dose		optional. Numeric.  Multiplier for dose
#			scale.rate		optional. Numeric.  Multiplier for rates
...
{	#	apc.plot.data.sums
	...
	{	sums.response	<- apc.data.sums(apc.data.list,data.type="r",average,keep.incomplete,apc.index)
		...
		if(data.format %in% data.format.list.regular)	{
				plot(seq(from=apc.index$age1,by=apc.index$unit[1],length=apc.index$age.max),sums.response$sums.age,main=main,xlab="age"   ,ylab=s.ylab,type=type,log=log,las=las)
				plot(seq(from=apc.index$per1,by=apc.index$unit[1],length=apc.index$per.max),sums.response$sums.per,main=main,xlab="period",ylab=s.ylab,type=type,log=log,las=las)
				plot(seq(from=apc.index$coh1,by=apc.index$unit[1],length=apc.index$coh.max),sums.response$sums.coh,main=main,xlab="cohort",ylab=s.ylab,type=type,log=log,las=las)
				title(main.outer,outer=TRUE)
		}	
		if(data.format %in% data.format.list.mixed)	{
			if(is.null(main.sub)==TRUE)	{
				if(apc.data.list$data.format %in% c("APm"))	{	main.row	<- "Response by age" 		}
				if(apc.data.list$data.format %in% c("PAm"))	{	main.row	<- "Response by period" 	}
				if(apc.data.list$data.format %in% c("PAm"))	{	main.col	<- "Response count by age" 		}
				if(apc.data.list$data.format %in% c("APm"))	{	main.col	<- "Response count by period" 	}
				if(is.null(apc.data.list$label)==FALSE)	{
					main.row	<- paste(main.row,", ",apc.data.list$label,sep="")
					main.col	<- paste(main.col,", ",apc.data.list$label,sep="")
					}
			}		
			series.row	<- rowSums(apc.data.list$response)
			series.col	<- colSums(apc.data.list$response)
			scale		<- scale.response
			dev.new();	plot(  series.col*scale,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.col,las=las);
			axis(side=1,at=1:length(series.col),labels=names(series.col));
			dev.new();	plot(  series.row*scale,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.row,las=las);
			axis(side=1,at=1:length(series.row),labels=names(series.row));
		}				
	}
	######################
	#	plot dose
	if(data.type %in% c("d","a") & is.null(apc.data.list$dose)==FALSE)	{
		...
		if(data.format %in% data.format.list.regular)	{
			plot(seq(from=apc.index$age1*scale.dose,by=apc.index$unit[1]*scale.dose,length=apc.index$age.max),sums.dose$sums.age,main=main,xlab="age"   ,ylab=s.ylab,type=type,log=log,las=las)
			plot(seq(from=apc.index$per1*scale.dose,by=apc.index$unit[1]*scale.dose,length=apc.index$per.max),sums.dose$sums.per,main=main,xlab="period",ylab=s.ylab,type=type,log=log,las=las)
			plot(seq(from=apc.index$coh1*scale.dose,by=apc.index$unit[1]*scale.dose,length=apc.index$coh.max),sums.dose$sums.coh,main=main,xlab="cohort",ylab=s.ylab,type=type,log=log,las=las)
			title(main.outer,outer=TRUE)
		}
		if(data.format %in% data.format.list.mixed)	{
			if(is.null(main.sub)==TRUE)	{
				if(apc.data.list$data.format %in% c("APm"))	{	main.row	<- "Dose count by age" 		}
				if(apc.data.list$data.format %in% c("PAm"))	{	main.row	<- "Dose count by period" 	}
				if(apc.data.list$data.format %in% c("PAm"))	{	main.col	<- "Dose count by age" 		}
				if(apc.data.list$data.format %in% c("APm"))	{	main.col	<- "Dose count by period" 	}
				if(is.null(apc.data.list$label)==FALSE)	{
					main.row	<- paste(main.row,", ",apc.data.list$label,sep="")
					main.col	<- paste(main.col,", ",apc.data.list$label,sep="")
					}
			}		
			series.row	<- rowSums(apc.data.list$dose)
			series.col	<- colSums(apc.data.list$dose)
			scale		<- scale.dose
			dev.new();	plot(  series.col*scale,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.col,las=las);
			axis(side=1,at=1:length(series.col),labels=names(series.col));
			dev.new();	plot(  series.row*scale,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.row,las=las);
	...
	if(data.type %in% c("m","a") & is.null(apc.data.list$dose)==FALSE)
	{
		...
		if(data.format %in% data.format.list.regular)	{
			plot(seq(from=(apc.index$age1*scale.rate),by=(apc.index$unit[1]*scale.rate),length=apc.index$age.max),sums.dose$sums.age,main=main,xlab="age"   ,ylab=s.ylab,type=type,log=log,las=las)
			plot(seq(from=(apc.index$per1*scale.rate),by=(apc.index$unit[1]*scale.rate),length=apc.index$per.max),sums.dose$sums.per,main=main,xlab="period",ylab=s.ylab,type=type,log=log,las=las)
			plot(seq(from=(apc.index$coh1*scale.rate),by=(apc.index$unit[1]*scale.rate),length=apc.index$coh.max),sums.dose$sums.coh,main=main,xlab="cohort",ylab=s.ylab,type=type,log=log,las=las)
			title(main.outer,outer=TRUE)
		}	
		if(data.format %in% data.format.list.mixed)	{
			if(is.null(main.sub)==TRUE)	{
				if(apc.data.list$data.format %in% c("APm"))	{	main.row	<- "Rates of sums by age" 		}
				if(apc.data.list$data.format %in% c("PAm"))	{	main.row	<- "Rates of sums by period" 	}
				if(apc.data.list$data.format %in% c("PAm"))	{	main.col	<- "Rates of sums by age" 		}
				if(apc.data.list$data.format %in% c("APm"))	{	main.col	<- "Rates of sums by period" 	}
				if(is.null(apc.data.list$label)==FALSE)	{
					main.row	<- paste(main.row,", ",apc.data.list$label,sep="")
					main.col	<- paste(main.col,", ",apc.data.list$label,sep="")
					}
			}		
			series.row	<- rowSums(apc.data.list$response)/rowSums(apc.data.list$dose)
			series.col	<- colSums(apc.data.list$response)/colSums(apc.data.list$dose)
			scale		<- scale.rate
			dev.new();	plot(  series.col*scale,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.col,las=las);
			axis(side=1,at=1:length(series.col),labels=names(series.col));
			dev.new();	plot(  series.row*scale,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.row,las=las);
	...
	######################
	#	plot crude rates
	if(data.type %in% c("c","a") & is.null(apc.data.list$dose)==FALSE)	{
		if(is.null(main.sub)==TRUE)	{
			if(data.format %in% c("AC","AP","trapezoid","trap","APm"))	{	main.row	<- "Crude rate by age" 		}
			if(data.format %in% c("PA","PC","PAm"))						{	main.row	<- "Crude rate by period" 	}
			if(data.format %in% c("CA","CP"))							{	main.row	<- "Crude rate by cohort" 	}
			if(data.format %in% c("CA","PA","PAm"))						{	main.col	<- "Crude rate by age" 		}
			if(data.format %in% c("AP","CP","APm"))						{	main.col	<- "Crude rate by period" 	}
			if(data.format %in% c("AC","PC","trapezoid","trap"))		{	main.col	<- "Crude rate by cohort" 	}
			if(is.null(apc.data.list$label)==FALSE)	{
				main.row	<- paste(main.row,", ",apc.data.list$label,sep="")
				main.col	<- paste(main.col,", ",apc.data.list$label,sep="")
			}
		}
		crude.rate.row	<- rowSums(apc.data.list$response)/rowSums(apc.data.list$dose)
		crude.rate.col	<- colSums(apc.data.list$response)/colSums(apc.data.list$dose)
		dev.new();	plot(  crude.rate.col*scale.rate,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.col,las=las);
			axis(side=1,at=1:length(crude.rate.col),labels=names(crude.rate.col));
		dev.new();	plot(  crude.rate.row*scale.rate,xaxt="n",xlab="",ylab="",type=type,log=log,main=main.row,las=las);
			axis(side=1,at=1:length(crude.rate.row),labels=names(crude.rate.row));			
	}	
	######################
}	#	apc.plot.data.sums
...
# 	{	sums.response	<- apc.data.sums(apc.data.list,data.type="r",average,keep.incomplete,apc.index)
# 		if(is.null(main.sub)==TRUE)	main	<- "response"
# 		plot(seq(from=apc.index$age1,by=apc.index$unit[1],length=apc.index$age.max),sums.response$sums.age,main=main,xlab="age"   ,ylab=s.ylab,type=type,log=log)	# 24 Jun 2025, unit updated
# 		plot(seq(from=apc.index$per1,by=apc.index$unit[1],length=apc.index$per.max),sums.response$sums.per,main=main,xlab="period",ylab=s.ylab,type=type,log=log)
# 		plot(seq(from=apc.index$coh1,by=apc.index$unit[1],length=apc.index$coh.max),sums.response$sums.coh,main=main,xlab="cohort",ylab=s.ylab,type=type,log=log)
# 	}
# 	######################
# 	#	plot dose
# 	if(data.type %in% c("d","a") & is.null(apc.data.list$dose)==FALSE)
...
# 		sums.dose		<- apc.data.sums(apc.data.list,data.type="d",average,keep.incomplete,apc.index)
# 		if(is.null(main.sub)==TRUE)	main	<- "dose"
# 		plot(seq(from=apc.index$age1,by=apc.index$unit[1],length=apc.index$age.max),sums.dose$sums.age,main=main,xlab="age"   ,ylab=s.ylab,type=type,log=log)		# 24 Jun 2025, unit updated
# 		plot(seq(from=apc.index$per1,by=apc.index$unit[1],length=apc.index$per.max),sums.dose$sums.per,main=main,xlab="period",ylab=s.ylab,type=type,log=log)
# 		plot(seq(from=apc.index$coh1,by=apc.index$unit[1],length=apc.index$coh.max),sums.dose$sums.coh,main=main,xlab="cohort",ylab=s.ylab,type=type,log=log)
# 	}
# 	######################
# 	#	plot mortality rates
# 	if(data.type %in% c("m","a") & is.null(apc.data.list$dose)==FALSE)
...
{	#	apc.plot.data.sparsity	
	...
	######################
	#	set limits	
	xlim	<- c(x1,x1 +(xmax-1)*unit[1])		# 24 Jun 2025, unit updated
	ylim	<- c(y1,y1 +(ymax-1)*unit[1])
	if(data.format=="CL")
		if(swap.axes)	xlim	<- c(x1 +(xmax-1)*unit[1],x1)		# 24 Jun 2025, unit updated
		else					ylim	<- c(y1 +(ymax-1)*unit[1],y1)	
	######################
	#	set plot array dimensions
	if(data.type %in% c("r","d") | is.null(apc.data.list$dose)==TRUE) 	par(mfrow=c(1,1))
	if(data.type == "a" & is.null(apc.data.list$dose)==FALSE) 			par(mfrow=c(1,2))
	...
	{	if(is.null(cex)==TRUE)
		...
		for(row in 1:l.xmax)
			for(col in 1:l.ymax)
			{
				x	<- l.x1+(row-1)*unit[1]		# 24 Jun 2025, unit updated
				y	<- l.y1+(col-1)*unit[1]
				z	<- data.matrix[row,col]
				if(is.na(z)==TRUE)				points(x,y,cex=cex,pch="x")
				else
...
{ 	#	apc.plot.data.within
	...
	######################
	#	plot
	v.x		<- seq(from=x1,length=x.max,by=apc.index$unit[1])		# 24 Jun 2025, unit updated
	...
	within	<- seq(from=w1,length=ceiling(w.max/l.thin),by=apc.index$unit[1]*l.thin)		# 24 Jun 2025, unit updated
...
{	#	apc.plot.data.level
	...
	x.lab<- as.character(x1+(x.at-1)*unit[1])		# 24 Jun 2025, unit updated
	if(l.rotate)	x.lab <- x.lab[length(x.lab):1]
	x	 <- list(at=x.at,labels=x.lab)
	y.at <- seq(from=1,to=ymax,length=lab[2])
	y.lab<- as.character(y1+(y.at-1)*unit[1])		# 24 Jun 2025, unit updated
