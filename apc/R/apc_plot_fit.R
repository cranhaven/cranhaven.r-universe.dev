...
{	#	apc.plot.fit
	...
	{	if(type=="detrend")
		...
		if(model.design %in% c("APC","AP","AC","PC","Ad","Pd","Cd","t","A","tA",l.model.F))
			l.dates[[4]]	<- age1+seq(0,age.max-1)*unit[1]		# 24 Jun 2025, unit updated
		if(model.design %in% c("P","tP"))									
			l.dates[[4]]	<- per1+seq(0,per.max-1)*unit[1]		# 24 Jun 2025, unit updated
		l.dates[[5]]	<- c(0,1)  # matrix(data=c(0,1)		     ,nrow=2	  ,ncol=1)
		l.dates[[6]]	<- coh1+seq(0,coh.max-1)*unit[1]			# 24 Jun 2025, unit updated
...
{	#	apc.plot.fit.pt
	...
	{	
		...
		####################
		#	limits for axes
		xlim	<- c(x1,x1 +(xmax-1)*unit[1])
		if(data.format=="CL")	ylim	<- c(y1 +(ymax-1)*unit[1],y1)		# 24 Jun 2025, unit updated
		else									ylim	<- c(y1,y1 +(ymax-1)*unit[1])
		####################
		#	plot parameters
		par(mfrow=c(1,1))		
		...
		for(row in 1:xmax)
			...
			{
				x	<- x1+(row	 -1)*unit[1]		#	get x coordinate		# 24 Jun 2025, unit updated
				y	<- y1+(column-1)*unit[1]		#	get y coordinate
