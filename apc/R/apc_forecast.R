...
{	#	apc.forecast.ac
	...
	apc.index.J	<- list(	age.max		=  age.max			,		
							...
							U			=  U				,
							n.data		=  n.data.J			,
							data.format = "trapezoid")             # BN 27 jun 2025 added
	#	get design matrix
	...
	if(distribution.forecasts)
	{
		...
		#	labels
		label.age	<- c(paste("age_",apc.internal.function.date.2.character(age1+seq(age.max-per.forecast.J,age.max-1)*unit[1],n.decimal),sep=""))		# 24 Jun 2025, unit updated	
		label.per	<- c(paste("per_",apc.internal.function.date.2.character(per1+seq(per.max,per.max+per.forecast.J-1)*unit[1],n.decimal),sep=""))
		label.coh	<- c(paste("coh_",apc.internal.function.date.2.character(coh1+seq(coh.max-per.forecast.J,coh.max-1)*unit[1],n.decimal),sep=""))
		label.all	<-  "all"                                                                               	
		#	forecasts
		...
		{	#	for intercept correction
			...
			label.sum	<- c(paste("per_",apc.internal.function.date.2.character(per1+seq(per.max,per.max+sum.length-1)*unit[1],n.decimal),sep=""))	# 24 Jun 2025, unit updated	
...
{	#	apc.forecast.ap
	...
	rownames(D.xi.per.extrapolated)	<- paste("D_period",apc.internal.function.date.2.character(per1+(per.max-1+seq(1,per.forecast.J))*unit[1],n.decimal),sep="")	# 24 Jun 2025, unit updated	
	#   I0 forecast	
...
#	#	derived values
	...
	#	add row names
	rownames(xi.per.dd.extrapolated)	<- paste("DD_period_",apc.internal.function.date.2.character(per1+(per.max-1+seq(1,per.forecast.J))*unit[1],n.decimal),"_x",sep="")	# 24 Jun 2025, unit updated	
...
{   #	apc.polygon 
  	x   <- seq(x.origin+unit[1],by=unit,length=nrow(m.forecast))	# 24 Jun 2025, unit updated	
