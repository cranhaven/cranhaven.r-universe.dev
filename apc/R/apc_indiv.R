...
apc.indiv.design.collinear <- function(data, unit=1, 
  ...
  i.value <- (data$age - age1)/unit[1] + 1 + n.age.excl.start		# BN 24 Jun 2025, unit updated
  k.value <- (data$cohort - coh1)/unit[1] + 1 + n.coh.excl.start
  ...
  age.names <- paste("DD_age",    seq(age1+2*unit[1], age1+2*unit[1]+(I-2)*unit[1]-1, unit[1]), sep="_")	# BN 24 Jun 2025, unit updated
  per.names <- paste("DD_period", seq(per1+2*unit[1], per1+2*unit[1]+(J-2)*unit[1]-1, unit[1]), sep="_")
  coh.names <- paste("DD_cohort", seq(coh1+2*unit[1], coh1+2*unit[1]+(K-2)*unit[1]-1, unit[1]), sep="_")
...
apc.indiv.fit.model <- function (apc.indiv.design.model, 
  ...
  if(difdif[1])	dates[index.age,1]	<- age1+seq(2,age.max-1)*unit[1]	# BN 24 Jun 2025, unit updated	
