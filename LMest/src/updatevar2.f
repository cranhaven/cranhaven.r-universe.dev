      subroutine updatevar2(Y,RR,n,TT,r,k,Mu,Si,Y1,Var)
...
c      integer indo1
      logical mask(r)
      double precision Y1(n,TT,r,k),Mu(n*TT,r,k),Si(r,r)
...
c check each row
     ...
     c                           matmul(Tmp,Si(indo,indm))
            do u = 1,k
              Y1(i,t,indm,u) = Mu(j,indm,u)+
     c                         matmul(Tmp,Y(i,t,indo)-Mu(j,indo,u))
            end do
          end if
        end do
      end do
