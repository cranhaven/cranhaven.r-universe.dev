#' @title Plot Linear Load Shape
#'
#' @description Plot method of \code{lslin} object



#' @param x An object of class \code{lslin}, created by
#' \code{\link{lslin}} function.
#'
...
#' @method plot lslin
#' @export
plot.lslin <- function(x, case = 1, col = c(1, 2),
                       ...
                       ... = NULL)
{
  ...
  if(scatter){
    ...
    graphics::plot(base, type = "l", col = col[1], lwd = tmp_lwd,
    ...
    graphics::grid(col = 1)
    graphics::mtext(side = 3, ln_ttl, font = 2, cex = 1, line = 1)
    graphics::mtext(side = 3, ln_sbttl, font=2, cex = 0.8, line = 0.3)
    if(legend){
