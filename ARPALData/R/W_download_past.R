...
W_download_past <- function(Metadata,Date_begin,Date_end,verbose = TRUE,
  ...
  for (yr in 1:dim(url_combs)[1]) {
    URLs[yr] <- url <- url_W_year_par(WPar = paste0(url_combs[yr,"WPar"],"_check"), Year = url_combs[yr,"Year"])
    temp <- tempfile()
    res <- suppressWarnings(try(curl::curl_fetch_disk(url, temp), silent = TRUE))
    url_combs$link1[yr] <- URLs[yr] <- url <- url_W_year_par(WPar = url_combs[yr,"WPar"], Year = url_combs[yr,"Year"])
