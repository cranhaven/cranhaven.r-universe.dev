...
ssvd_WIN <- function(){     # Change newmethod to your own method name
	...
	other.arg <- ",method=BCssvd()"  # Comma in the beginning but not at the end ! 
	...
	frame.name <- "entrygeneral"  
	argument.names <- c("Number of SSVD layers (BC's)","Convergence Threshold","Maximum number of iterations") 
	argument.types <- c("num","num","num") 
	arguments <- c("K","merr","niter") 
	initial.values <- c(10,1e-04,100)
	...
	# Change variables accordingly:
	frame.name <- "radioleftsingular"
	argument.names <- c("(Adaptive) LASSO","Hard Thresholding")  
	arguments <- c("threu")		
	...
	# Change variables accordingly:
	frame.name <- "entryleftsingular"  
	argument.names <- c("Weight Parameter in Adaptive LASSO (>=0 ; 0 = LASSO)") 
	argument.types <- c("num") 
	arguments <- c("gamu") 
	initial.values <- c(0)
	title <- ""
	...
	grid.config <- .grid.matrix(input=input,c("entrygeneral","radioleftsingular","entryleftsingular","radiorightsingular","entryrightsingular"),byrow=TRUE,nrow=5,ncol=1,grid.config=grid.config)
	...
	grid.rows <- .combine.rows(input=input,rows=c(2,3),title="Left Singular Vector Parameters",border=TRUE,grid.rows=grid.rows,grid.config=grid.config)
	grid.rows <- .combine.rows(input=input,rows=c(4,5),title="Right Singular Vector Parameters",border=TRUE,grid.rows=grid.rows,grid.config=grid.config)
