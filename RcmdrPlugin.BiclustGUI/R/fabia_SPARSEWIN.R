...
fabiaSPARSE_WIN <- function(){     # Change newmethod to your own method name
	...
	argument.names <- c("Number of Biclusters","Number of Iterations","Sparsness Loadings [0,1]","Sparsness Prior Loadings [0,2]","Sparsness Factors [0.5,2]","Random init. loadings (<0 SVD ; >0 [-r,r])") 
	argument.types <- c("num","num","num","num","num","num")
	arguments <- c("p","cyc","alpha","spl","spz","random") 
	initial.values <- c(13,500,0.01,0,0.5,1)
	title <- ""
	border <- FALSE
	entry.width <- c("4","4","4","4","4","4")  
	...
	argument.names <- c("Scale Loading Vectors","Min. value of variational pm.","Max. biclusters for row (default = no limit)","Max. rows per bicluster (default = no limit)","Cycle Start (default = beginning)") 
	argument.types <- c("num","num","num","num","num")
	arguments <- c("scale","lap","nL","lL","bL") 
	initial.values <- c(0,1,0,0,0)
	title <- ""
	border <- FALSE
	...
	# Change variables accordingly:
	frame.name <- "specialentryframe1"  
	argument.names <- c("Vector of Samples (def.=ALL)","Index Vector to init. L (def.=random)","Number of Iterations","Quantile L Values removal") 
	argument.types <- c("num","num","num","num")
	arguments <- c("samples","initL","iter","quant") 
	initial.values <- c(0,0,1,0.001)
	title <- ""
	border <- FALSE
	entry.width <- c("6","6","6","6")  
	...
	# Change variables accordingly:
	frame.name <- "specialentryframe2"  
	argument.names <- c("Lower Bound filter col.","Upper Bound filter col.","Lower Bound Variational lapla","Lower Bound for divisions") 
	argument.types <- c("num","num","num","num")
	arguments <- c("lowerB","upperB","eps","eps1") 
	initial.values <- c(0.0,1000.0,1e-3,1e-10)
	title <- ""
	border <- FALSE
	entry.width <- c("6","6","6","6")  
	...
	# Change variables accordingly:
	frame.name <-  "specialcheckframe"
	argument.names <- c("Rescale Z, L factors","Compute bicluster IC") 
	arguments <- c("dorescale","doini") 
	...
	# Change variables accordingly:
	frame.name <- "datatypeframe"
	argument.names <- c("Existing Sparse Matrix file","Convert Dataset to SPARSE file")   
	arguments <- c("datatype")		
	argument.types <- "num"  
	...
	argument.names <- c("Filename") 
	argument.types <- c("char")
	arguments <- c("filename") 
	initial.values <- c("data")
	title <- ""
	border <- FALSE
	entry.width <- c("17")
	...
	# Change variables accordingly:
	frame.name <-  "writecheck"
	argument.names <- c("Write Result Files") 
	arguments <- c("write_file") 
	initial.values <- c(0) 
	title <- ""
	border <- FALSE
	...
	grid.config <- .grid.matrix(input=input,c("entryframe1","entryframe2",NA,"nonnegframe",NA,NA,"specialentryframe1","specialentryframe2",NA,"specialcheckframe",NA,NA ,"datatypeframe","fileframe"  ,"normframe","writecheck",NA,NA),byrow=TRUE,nrow=6,ncol=3,grid.config=grid.config)
	...
	grid.rows <- .combine.rows(input=input,rows=c(3,4),title="Extra SPARSE Options",border=TRUE,grid.rows=grid.rows,grid.config=grid.config)
	grid.rows <- .combine.rows(input=input,rows=c(5,6),title="Data Manipulation",border=TRUE,grid.rows=grid.rows,grid.config=grid.config)
