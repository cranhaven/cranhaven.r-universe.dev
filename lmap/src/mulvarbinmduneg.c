...
double mulvarbinmduneg( const size_t n, const size_t r, double** y, const size_t m, double** u, double** v, const bool mains, double* mu, const size_t MAXINNER, const double FCRIT, const size_t MAXITER, const double DCRIT, size_t* lastiter, double* lastdif )
// mulvarbinmduneg() performs multivariate binary multidimensional unfolding
{
  // constants
  ...
  for ( iter = 1; iter <= MAXITER; iter++ ) {
    ...
    size_t inner = 0;
    double fdif = 0.0;
    mduneg( n, r, delta, m, u, fu, v, fv, d, MAXINNER, FCRIT, &inner, &fdif, false );
...
void Cmulvarbinmduneg( int* rn, int* rr, double* ry, int* rm, double* ru, double* rv, int* rmains, double* rmu, int* rmaxinner, double* rfcrit, int* rmaxiter, double* rdcrit, double* rdeviance )
// Cmulvarbinmduneg() performs multivariate binary multidimensional unfolding.
{
  // transfer to C
  const size_t n = *rn;
  const size_t r = *rr;
  ...
  size_t lastiter = 0;
  double lastdif = 0.0;
  const double dnew = mulvarbinmduneg( n, r, y, m, u, v, mains, mu, MAXINNER, FCRIT, MAXITER, DCRIT, &lastiter, &lastdif );
