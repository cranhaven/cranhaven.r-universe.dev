...
#' 
#' \code{\link{gofKendallCvM}} tests a given dataset for a copula based on
...
#' the estimation can be given. The possible copulae are \code{"normal"}, 
#' \code{"t"}, \code{"clayton"}, \code{"gumbel"}, \code{"frank"}, \code{"joe"}, 
#' \code{"amh"}, \code{"galambos"}, \code{"huslerReiss"}, \code{"tawn"}, 
#' \code{"tev"}, \code{"fgm"} and \code{"plackett"}. See for
...
#' u_d).}{1/n sum(U[i1] <= u_1, ..., U[id] <= u_d, i = 1, ..., n).} Let the
#' rescaled pseudo observations be \eqn{V_1 = C_n(U_1), \dots, V_n =
#' C_n(U_n)}{V[1] = Cn(U[1]), ..., V[n] = Cn(U[n]} and the distribution
#' function of \eqn{V}{V} shall be \eqn{K}. The estimated version is given by
#' \deqn{K_n(v) = \frac{1}{n} \sum_{i=1}^n \mathbf{I}(V_i \leq v)}{Kn(v) = 1/n
#' sum(V[i] <= v, i = 1, ..., n)} with \eqn{v \in [0,1]^d.}{v in [0,1]^d.} The
#' testable \eqn{H_0^{'}}{H0^'} hypothesis is then \deqn{K \in \mathcal{K}_0 =
#' \{K_{\theta} : \theta \in \Theta \}}{K in K_0 = {Ktheta : theta in Theta}}
#' with \eqn{\Theta}{Theta} being an open subset of \eqn{R^p}{R^p} for an
#' integer \eqn{p \geq 1}{p >= 1}, see Genest et al. (2009). The resulting
#' Cramer-von Mises test statistic is then given by \deqn{T = n \int_0^1
#' (K_n(v) - K_{\theta_n})^2 d K_{\theta_n}(v).}{T = n int_0^1 (Kn(v) -
#' Kthetan)^2 d Kthetan(v).}
#' 
#' Because \eqn{H_0^{'}}{H0^'} consists of more distributions than
#' \eqn{H_0}{H0} the test is not necessarily consistent.
#' 
...
#' \doi{10.1016/j.insmatheco.2007.10.005}\cr \cr Christian
...
#' Transformation. \emph{Scandinavian Journal of Statistics, Volume 33, Issue
#' 2, 2006, Pages 337-366}.
#' \doi{10.1111/j.1467-9469.2006.00470.x}\cr \cr Ulf
#' Schepsmeier, Jakob Stoeber, Eike Christian Brechmann, Benedikt Graeler
#' (2015). VineCopula: Statistical Inference of Vine Copulas. \emph{R package
#' version 1.4.}. \url{https://cran.r-project.org/package=VineCopula}
#' @examples
#' 
#' data(IndexReturns2D)
#' 
...
#' 
#' @export gofKendallCvM
gofKendallCvM <- function(copula = c("normal", "t", "clayton", "gumbel", 
                                     "frank", "joe", "amh", "galambos", 
                                     "huslerReiss", "tawn", "tev", "fgm", 
                                     "plackett"), 
                          x, param = 0.5, 
