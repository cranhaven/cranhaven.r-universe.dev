...
#' @param customTest A character string with the name of the customized test.
...
#' 
#' 
#' @export gofCustomTest
gofCustomTest <- function(copula = c("normal", "t", "clayton", "gumbel", 
                                     "frank", "joe", "amh", "galambos", 
                                     "huslerReiss", "tawn", "tev", "fgm", 
                                     "plackett"), x, customTest = NULL, 
...
stop(
"The name of the custom test has to be specified. The test has to be present in 
the workspace of your R session."
)
  }
  if (length(customTest) > 1) {
stop(
"Please only submit one 'customTest' at a time. For simultaneous derivation of 
several custom tests, please use the 'gof' function."
)
  }
  if (!is.element(customTest, ls(".GlobalEnv"))) {
stop(
"The function defined in 'customTest' cannot be found in the workspace. Please 
load the function."
)
  }
  if (any(!vapply(customTest, 
...
numbers."
  ...
  res <- try(.gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = customTest, estim.method = "mpl", 
...
Therefore df.est was set to FALSE for the bootstrapping."
    ...
    res.f <- .gofCopulapb(copula = erg[[1]], x = erg[[2]], M = M, 
                          method = customTest, estim.method = "itau", 
                          processes = processes, param.est = param.est, 
