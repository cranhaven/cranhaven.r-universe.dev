...
* the function `juliaOptions()` has been added to allow specification of  parameters for the Julia evaluator in the style of the `options()` function.  The parameter `largeObject` can be used to set the corresponding field in an evaluator (by default the current evaluator).
