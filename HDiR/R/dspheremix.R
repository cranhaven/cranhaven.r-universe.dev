dspheremix<-function (x, model = NULL){
    if(!is.matrix(x)|(ncol(x)!=3)|any(is.wholenumber(apply((x)**2,1,sum))!=1))
	  stop("argument 'x' must be a matrix of 3 columns of points on the unit sphere")
    if (is.null(model))
...
return(dmovMF(x,k*mu,alpha=prob))
}
