...
#'
#' @param tnrs A `data.frame`, usually an output from [tnrs_match()] or [rotl::tnrs_match_names()], but see details.
...
clean_tnrs <- function(tnrs, invalid = c("barren", "extinct", "uncultured", "major_rank_conflict", "incertae", "unplaced", "conflict", "environmental", "not_otu"),
  ...
  if (!"flags" %in% names(tnrs)) {
    message("... tnrs should be a data.frame from 'rotl::tnrs_match_names' function")
