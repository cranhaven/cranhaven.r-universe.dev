...
#' @param x A Nasdaq Data Link time series object or search results with attached meta data.
#' @return Returns a list of meta data about the series or search results.
#' @seealso \code{\link{NasdaqDataLink.api_key}}, \code{\link{NasdaqDataLink.search}}
#' @examples \dontrun{
#' metaData(ts)
#' }
#' @export
...
#'
#' @param code Dataset code on Nasdaq Data Link specified as a string or an array of strings.
#' @param type Type of data returned specified as string. Can be 'raw', 'ts', 'zoo', 'xts' or 'timeSeries'.
#' @param transform Apply Nasdaq Data Link API data transformations.
...
#' @param ... Additional named values that are interpreted as Nasdaq Data Link API parameters. Please see \url{https://docs.data.nasdaq.com/docs/in-depth-usage#get-time-series-metadata} for a full list of parameters.
#' @return Depending on the type the class is either data.frame, time series, xts, zoo or timeSeries.
#' @references This R package uses the Nasdaq Data Link API. For more information go to \url{https://docs.data.nasdaq.com}. For more help on the package itself go to \url{https://data.nasdaq.com/tools/r}.
#' @seealso \code{\link{NasdaqDataLink.api_key}}
#' @examples \dontrun{
#' data_link_data = NasdaqDataLink("NSE/OIL", collapse="monthly", start_date="2013-01-01", type="ts")
...
#' @export
NasdaqDataLink <- function(code, type = c("raw", "ts", "zoo", "xts", "timeSeries"),
  ...
  if (length(code) == 1) {
    ...
    # download data
    data <- NasdaqDataLink.dataset.get(code, params)
    if (params$collapse != "") {
    ...
    for (c in code) {
      ...
      merge_data <- tryCatch(NasdaqDataLink.dataset.get(c, tmp.params), error = function(e) {
...
#' @details Set your \code{api_key} with \code{NasdaqDataLink.api_key} function. For instructions on finding your api key go to \url{https://data.nasdaq.com/account/profile}
#'
#' @param code Dataset code on Nasdaq Data Link specified as a string.
#' @param params A list of parameters to be passed to the Nasdaq Data Link API. Please see \url{https://docs.data.nasdaq.com/docs/in-depth-usage#get-time-series-metadata} for a full list of parameters.
#' @return Returns a data.frame of the requested data
#' @seealso \code{\link{NasdaqDataLink.api_key}}, \code{\link{NasdaqDataLink}}
#' @examples \dontrun{
#' data_link_data = NasdaqDataLink.dataset.get("NSE/OIL", list(rows=5))
#' plot(data_link_data[,1])
#' }
#' @export
NasdaqDataLink.dataset.get <- function(code, params) {
  ...
  json <- do.call(nasdaq_data_link.api, c(path = path, params))$dataset
