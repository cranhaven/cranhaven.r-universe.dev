...
context("NasdaqDataLink() bad argument errors")
test_that("Invalid transform throws error", {
  expect_error(NasdaqDataLink("NSE/OIL", transform = "blah"))
})

test_that("Invalid collapse throws error", {
  expect_error(NasdaqDataLink("NSE/OIL", collapse = "blah"))
})

test_that("Invalid type throws error", {
  expect_error(NasdaqDataLink("NSE/OIL", type = "blah"))
})

context("NasdaqDataLink() call")
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
    test_that("correct arguments are passed in", {
      expect_equal(http, "GET")
      expect_equal(url, "https://data.nasdaq.com/api/v3/datasets/NSE/OIL")
      expect_null(body)
  ...
  NasdaqDataLink("NSE/OIL", transform = "rdiff", collapse = "annual", start_date = "2015-01-01")
)

context("NasdaqDataLink() daily data response")
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
    mock_response()
  },
  ...
  test_that("list names are set to column names", {
    dataset <- NasdaqDataLink("NSE/OIL")
  ...
  test_that("does contain meta attribute when requested", {
    dataset <- NasdaqDataLink("NSE/OIL", meta = TRUE)
...
with_mock(
  ...
  test_that("zoo is returned instead of ts if ts is not supported for frequency", {
    dataset <- NasdaqDataLink("NSE/OIL", type = "ts")
    expect_is(dataset, "zoo")
  }),
  test_that("Data is the same across formats", {
    annaulraw <- NasdaqDataLink("NSE/OIL", type="raw", collapse = "annual")
    annaults <- NasdaqDataLink("NSE/OIL", type="ts", collapse = "annual")
    annaulzoo <- NasdaqDataLink("NSE/OIL", type="zoo", collapse = "annual")
    annaulxts <- NasdaqDataLink("NSE/OIL", type="xts", collapse = "annual")
    annaultimeSeries <- NasdaqDataLink("NSE/OIL", type="timeSeries")
...
context("NasdaqDataLink() annual collapse response data")
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
    mock_response(content = mock_annual_data())
  },
  ...
  test_that("return ts when requested", {
    dataset <- NasdaqDataLink("NSE/OIL", type = "ts", collapse = "annual")
    expect_is(dataset, "ts")
  }),
  test_that("When requesting xts annual warn user xts has non-standard meaning for frequency", {
    expect_warning(NasdaqDataLink("NSE/OIL", type="xts", collapse = "annual"),
      "xts has a non-standard meaning for 'frequency'.", fixed = TRUE)
  })
)

context("NasdaqDataLink() monthly collapse response data")
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
  ...
  test_that("Frequencies are correct across output formats", {
    monthlyts <- NasdaqDataLink("NSE/OIL", type="ts", collapse = "monthly")
    monthlyzoo <- NasdaqDataLink("NSE/OIL", type="zoo", collapse = "monthly")
    monthlyxts <- NasdaqDataLink("NSE/OIL", type="xts", collapse = "monthly")
    monthlytimeSeries <- NasdaqDataLink("NSE/OIL", type="timeSeries", collapse = "monthly")
...
context("NasdaqDataLink() annual frequency response data")
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
  ...
  test_that("Start and end dates are correct (zoo)", {
    annual <- NasdaqDataLink("NSE/OIL", type="zoo")
    ...
    annual <- NasdaqDataLink("NSE/OIL", type="zoo", force_irregular = TRUE)
  ...
  test_that("Start and end dates are correct (xts)", {
    annual <- NasdaqDataLink("TESTS/4", type="xts")
  ...
  test_that("Start and end dates are correct (timeSeries)", {
   annual <- NasdaqDataLink("TESTS/4", type="timeSeries")
...
context("NasdaqDataLink() quarterly frequency response data")
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
  ...
  test_that("Collapsed data frequency", {
    dailytoquart <- NasdaqDataLink("TESTS/1", type="ts", collapse="quarterly")
    expect_equal(frequency(dailytoquart), 4)
  })
)

context('NasdaqDataLink() filtering by column index')
with_mock(
  `httr::VERB` = function(http, url, config, body, query) {
    test_that("correct arguments are passed in", {
  ...
  NasdaqDataLink("NSE/OIL.1")
)

context('NasdaqDataLink() multiple datasets')
test_that("Multiple dataset codes with dataset code column indexes are requested", {
  ...
  with_mock(
    ...
    NasdaqDataLink(c("NSE/OIL.1", "AAPL/WIKI.2"), transform = "rdiff", column_index = 3)
  )
  expect_equal(i, 2)
})
...
test_that("Multiple dataset codes calls merge", {
  ...
  with_mock(
    ...
    NasdaqDataLink(c("NSE/OIL.1", "WIKI/AAPL.2", "TESTS/4.1"))
  )
...
test_that("Multiple dataset codes returns desired requested type", {
  with_mock(
    ...
    },
    {
      ...
      for(type in types) {
        i <- i + 1
        dataset = NasdaqDataLink(c("NSE/OIL.1", "WIKI/AAPL.2", "TESTS/4.1"), type = type, collapse = 'annual')
