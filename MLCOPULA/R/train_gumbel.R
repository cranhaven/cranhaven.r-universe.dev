#' @title Train a classification model using the Gumbel copula.
...
train.gumbel <- function(X, y, distribution,weights,graph_model){
  nc <- length(unique(y))
  est <- list()
  for(c in 0:(nc-1)){
    Xc <- X[y == c,]
    ...
    #Estimar copulas
    est_cop <- build.weights(den_est$U, cop.est = estimation.gumbel,cop = "gumbel",
                             weights = weights)
