#' @title Train a classification model using the Clayton copula.
...
train.clayton <- function(X, y, distribution,weights,graph_model){
  nc <- length(unique(y))
  est <- list()
  for(c in 0:(nc-1)){
    Xc <- X[y == c,]
    ...
    #Estimar copulas
    est_cop <- build.weights(den_est$U, cop.est = estimation.clayton,
                             cop = "clayton",weights = weights)
    #
    if(graph_model == "tree"){
