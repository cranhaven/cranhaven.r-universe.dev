plot_traceplots_param <-
  function(mcmc, what = "ng", yspec, burnin = 0, thin = 1, iterations, chains, setparmfrow = TRUE, allclusters = FALSE){
  ...
  if(!is.element(what, rownames(mcmc$settings[[chains[1]]])[mcmc$settings[[chains[1]]]$save])){
    stop(paste0("Parameter ", what, " is not saved."))
  }
          ...
          for(ch in for_chains){
            for(g in nonempty[[ch]]){
              ...
              for(i in 1:v$d1){
                for(j in 1:v$d2){
                  if(i <= j){
                    plot_traceplots(mcmc = mcmc, what = what, 
      ...
      if(v$gspec){
          ...
          if(v$isy){
            ...
            for(ch in for_chains){
              ...
              for(g in nonempty[[ch]]){
                plot_traceplots(mcmc = mcmc, what = what,
                                gspec = g,
                                burnin = burnin, thin = thin, chains = plot_chains[[ch]],
        ...
        if(v$D == 1){
          ...
          if(v$ydepd1){
            ...
            # the size of the dimension does NOT depend on outcomes
            for(ch in for_chains){
