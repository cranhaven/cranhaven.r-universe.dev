...
#' @import ivreg
#' @export
mr_2sls <- function(df = NULL, prs_col = "SCORESUM", exposure_col = NA,
  ...
  if (verbose) cat("\n\n---\nMendelian Randomization Two-Stage Least Square (2SLS):", file = log, append = TRUE)
  ...
  formula <- paste0("`",outcome_col,"` ~ `", exposure_col, "`|`",prs_col,"`")
  exposure_formula <- paste0("`",exposure_col,"` ~ `",prs_col,"`")

  # doing 2SLS method using ivreg, it is better to truly estimate SE
  g <- ivreg(formula, x = TRUE, data = df)

  # checking the Outcome info according to phenotype type
  sample_size <- nrow(df)
  if (outcome_type == "Cases/Controls") {
  ...
  beta <- summary(g)$coef[2] #2SLS estimate
  se <- summary(g)$coef[2,2] #Standard error of the 2SLS estimate
  fstat <- summary(lm(exposure_formula, data = df))$fstatistic[1] #f stat
