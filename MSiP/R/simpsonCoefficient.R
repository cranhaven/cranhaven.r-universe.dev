...
#' @return Data frame containing bait-prey pairs with Simpson coefficient,
...
#' @description This function computes the Simpson similarity coefficient scores for
...
simpsonCoefficient <- function(datInput) {
    ...
    simpson.sim <-
        lapply(pair.list, function(x){
            (length(which(x[[1]] & x[[2]])))/pmin(length(which(x[[1]] != 0)),length(which(x[[2]] != 0)))
        }
        )
