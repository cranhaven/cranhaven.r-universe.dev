two.step.hybrid.2d <-function(folder, info, min.within.batch.prop.detect=0.4, min.within.batch.prop.report=0.5, min.batch.prop=0.5, batch.align.mz.tol=1e-5, batch.align.chr.tol=50, file.pattern=".cdf", known.table=NA, n.nodes=4, mz.cut = 1e-4, rt.cut = 50, mz.search.range =  5e-4, rt.search.range = 200,  intensity.limit.quantile = 0.05, mPower=4, mz.tol=1e-5, align.mz.tol=NA, align.chr.tol=NA, max.align.mz.diff=0.01, pre.process=FALSE, recover.mz.range=NA, recover.chr.range=NA, use.observed.range=TRUE, match.tol.ppm=NA, new.feature.min.count=2, recover.min.count=3)
{
    oldwd <- getwd()
    on.exit(setwd(oldwd))
    setwd(folder)
    ...
    for(batch.i in 1:length(batches))
    {
		...
		b<-semi.sup.2d(folder, folder, file.pattern=file.pattern, known.table=known.table, n.nodes=n.nodes, min.exp=ceiling(min.within.batch.prop.detect*length(this.subs)), mz.cut = mz.cut, rt.cut = rt.cut, mz.search.range = mz.search.range, rt.search.range = rt.search.range,  intensity.limit.quantile = intensity.limit.quantile, mPower=mPower, mz.tol=mz.tol, subs=this.subs, align.mz.tol=align.mz.tol, align.chr.tol=align.chr.tol, max.align.mz.diff=max.align.mz.diff, recover.mz.range=recover.mz.range, recover.chr.range=recover.chr.range,use.observed.range=use.observed.range, new.feature.min.count=new.feature.min.count, recover.min.count=recover.min.count)
	...
	for(batch.i in 1:length(batches))
    {
		...
		{
			r<-recover.weaker(filename=colnames(this.aligned)[i], loc=i, aligned.ftrs=that.aligned, pk.times=that.pk.time, align.mz.tol=batch.align.mz.tol, align.chr.tol=batch.align.chr.tol, this.f1=batchwise[[batch.i]]$features[[i]], this.f2=batchwise[[batch.i]]$features2[[i]], mz.range=recover.mz.range, chr.range=recover.chr.range, use.observed.range=use.observed.range, orig.tol=mz.tol, bandwidth=.5, recover.min.count=recover.min.count)
