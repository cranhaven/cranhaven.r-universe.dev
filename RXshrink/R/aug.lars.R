...
{ 
    ...
    if (missing(data) || !inherits(data, "data.frame")) 
        stop("Second argument to aug.lars must be an existing Data Frame.")
    dfname <- deparse(substitute(data))
    if (!is.element(yvar, dimnames(data)[[2]])) 
        stop("Response variable in the aug.lars formula must be an existing variable.")
    lmobj <- lm(form, data)
...
function (x, trace = "all", trkey = FALSE, ...) 
{
    ...
    if (trace == "all" || trace == "seq" || trace == "coef") {
        ...
        if( trkey )
            legend("bottom",all.vars(x$form)[2:(x$p+1)], col=1:(x$p), lty=myty[1:(x$p)], lwd=2)
    }
    if (trace == "seq") {
    ...
    if (trace == "all" || trace == "seq" || trace == "exev") {
        ...
        if( trkey )
            legend("bottom",paste("Component",1:(x$p)), col=1:(x$p), lty=myty[1:(x$p)], lwd=2)
    }
    if (trace == "seq") {
