nested_Gsq_nr <- function(y, strata, fixed.strata, h0.fct, h0.fct.deriv, S0.fct, S0.fct.deriv, max.mph.iter,
  ...
  nested.Gsq.CI.high <- NULL
  nested.Gsq.CI.low <- NULL
  if (!is.null(S.space.H0)) {
    ...
    if (abs(S0.fct.m_H0 - upper.boundary) < tol.psi) {
      nested.Gsq.CI.high <- upper.boundary
    }
    if (abs(S0.fct.m_H0 - lower.boundary) < tol.psi) {
      nested.Gsq.CI.low <- lower.boundary
    }
  }
  # compute nested.Gsq.CI.high
  if (is.null(nested.Gsq.CI.high)) {
    ...
    f.psi.high.list[1] <- f.psi(y, strata = strata, fixed.strata = fixed.strata, h0.fct = h0.fct,
                                ...
                                S0.fct.deriv = S0.fct.deriv, method_specific = "nested.Gsq",
    ...
    nested.Gsq.CI.high <- psi.high.list[iter.count+3]
  }
  # compute nested.Gsq.CI.low
  if (is.null(nested.Gsq.CI.low)) {
    ...
    nested.Gsq.CI.low <- psi.low.list[iter.count+3]
  }
  nested.Gsq.CI <- sort(c(nested.Gsq.CI.low, nested.Gsq.CI.high))
  t(nested.Gsq.CI)
}
