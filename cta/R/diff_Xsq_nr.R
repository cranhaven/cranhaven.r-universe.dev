diff_Xsq_nr <- function(y, strata, fixed.strata, h0.fct, h0.fct.deriv, S0.fct, S0.fct.deriv, max.mph.iter,
  ...
  Xsq_H0 <- mph.fit_H0$Xsq     # Xsq under H0
  diff.Xsq.CI.high <- NULL
  diff.Xsq.CI.low <- NULL
  if (!is.null(S.space.H0)) {
    ...
    if (abs(S0.fct.m_H0 - upper.boundary) < tol.psi) {
      diff.Xsq.CI.high <- upper.boundary
    }
    if (abs(S0.fct.m_H0 - lower.boundary) < tol.psi) {
      diff.Xsq.CI.low <- lower.boundary
    }
  }
  # compute diff.Xsq.CI.high
  if (is.null(diff.Xsq.CI.high)) {
    ...
    f.psi.high.list[1] <- f.psi(y, strata = strata, fixed.strata = fixed.strata, h0.fct = h0.fct,
                                ...
                                S0.fct.deriv = S0.fct.deriv, method_specific = "diff.Xsq",
                                ...
                                max.score.diff.iter = max.score.diff.iter, pdlambda = NULL, Xsq_H0 = Xsq_H0)
    ...
    diff.Xsq.CI.high <- psi.high.list[iter.count+3]
  }
  # compute diff.Xsq.CI.low
  if (is.null(diff.Xsq.CI.low)) {
    ...
    diff.Xsq.CI.low <- psi.low.list[iter.count+3]
  }
  diff.Xsq.CI <- sort(c(diff.Xsq.CI.low, diff.Xsq.CI.high))
  t(diff.Xsq.CI)
}
