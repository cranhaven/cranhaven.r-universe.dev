diff_Gsq_nr <- function(y, strata, fixed.strata, h0.fct, h0.fct.deriv, S0.fct, S0.fct.deriv, max.mph.iter,
  ...
  Gsq_H0 <- mph.fit_H0$Gsq     # Gsq under H0
  diff.Gsq.CI.high <- NULL
  diff.Gsq.CI.low <- NULL
  if (!is.null(S.space.H0)) {
    ...
    if (abs(S0.fct.m_H0 - upper.boundary) < tol.psi) {
      diff.Gsq.CI.high <- upper.boundary
    }
    if (abs(S0.fct.m_H0 - lower.boundary) < tol.psi) {
      diff.Gsq.CI.low <- lower.boundary
    }
  }
  # compute diff.Gsq.CI.high
  if (is.null(diff.Gsq.CI.high)) {
    ...
    f.psi.high.list[1] <- f.psi(y, strata = strata, fixed.strata = fixed.strata, h0.fct = h0.fct,
                                ...
                                S0.fct.deriv = S0.fct.deriv, method_specific = "diff.Gsq",
                                ...
                                max.score.diff.iter = max.score.diff.iter, pdlambda = NULL, Gsq_H0 = Gsq_H0)
    ...
    diff.Gsq.CI.high <- psi.high.list[iter.count+3]
  }
  # compute diff.Gsq.CI.low
  if (is.null(diff.Gsq.CI.low)) {
    ...
    diff.Gsq.CI.low <- psi.low.list[iter.count+3]
  }
  diff.Gsq.CI <- sort(c(diff.Gsq.CI.low, diff.Gsq.CI.high))
  t(diff.Gsq.CI)
}
