#' The global lambda for models 2 and 3 
#'
...
#'
#' @export
Lamb_grl <- function(times, subj, X, y, d, tau, kn, degree, lambda,range){
  dim = length(subj)
  X = matrix(X, nrow = dim)
  px = ncol(X)-1
  ...
  plam = NULL
  for (i in 1:nlam) {
    qvcsic = qrvcp_grl(times, subj, y, X, tau, kn, degree, 
    ...
    yhatsic[, i] = rowSums(yhatsic_k)+hat_btsic[px*dim+1]* X[, px+1]
