...
#' @description Simulate data from bivariate correlated gamma or lognormal frailty models with or without covariates.
#'
...
#' }
#'
simbcfrailph<-function(psize, cenr= c(0),beta=c(0.5),frailty,frailpar=c(0.5,0.25),
bhaz=c("weibull"),bhazpar=list(shape =c(0.5), scale = c(0.01)),
covartype= c("B"),covarpar=list(fargs=c(1),sargs=c(0.5)),inpcovar=NULL,inpcen=NULL,comncovar=NULL){
Call <- match.call()
...
if(frailpar[1]<=0){stop("frailty variance parameter is invalid")}
if(frailpar[2]<0){
if(frailty=="gamma"){
stop("gamma frailty not allowed negative correlation")}}
if (frailpar[2]>1) {stop(" invalid frailty correlation parameter")}
...
gdataa=genbcfrail(psize,cenr,cent1,cent2,beta,frailty,fpar,bhaz,bhazpar,
covartype,covarpar,inputcovar,covar1,covar2,comncovar=comncovar)
dataa<-gdataa$dataa;Y<-gdataa$Y;X<-gdataa$X
gendat <-list(data=dataa,Y=Y,X=X,numberofpair=psize,
frailty=frailty,frailpar=frailpar,basehaz=bhaz,
covartype=covartype,covarcoef=beta)
gendat$call <- match.call()
class(gendat) <- c("simbcfrailph")
gendat}
