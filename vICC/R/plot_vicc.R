...
plot.vicc <- function(x , type = "coef", ...) {
  ...
  if (type == "coef") {
    if (x$type == "customary") {
      icc1 <-
        mean((samps$tau_mu ^ 2 / (samps$tau_mu ^ 2 + (samps$sigma) ^ 2)))

      icc2 <-
        mean((samps$tau_mu ^ 2 / (
          samps$tau_mu ^ 2 + (samps$sigma) ^ 2 / mean(obs_per_group)
        )))
      ...
      icc1 <-
        mean((samps$tau_mu ^ 2 / (
          samps$tau_mu ^ 2 + exp(samps$fe_sd) ^ 2
        )))

      icc2 <-
        mean((samps$tau_mu ^ 2 / (
          samps$tau_mu ^ 2 + exp(samps$fe_sd) ^ 2 / mean(obs_per_group)
        )))
      ...
      dat$group_color <-
        as.factor(ifelse(dat$Cred.lb < fixef(x)[2, 1] &
                           dat$Cred.ub > fixef(x)[2, 1], 0 , 1))

      plot_sd <- ggplot(dat, aes(
        x = as.factor(group),
        y = Post.mean,
        color = group_color
      )) +
      ...
      dat$group_color <-
        as.factor(ifelse(dat$Cred.lb < icc1 &
                           dat$Cred.ub > icc1, 0 , 1))
      ...
      dat$group_color <-
        as.factor(ifelse(dat$Cred.lb < icc2 &
                           dat$Cred.ub > icc2, 0 , 1))
    ...
    if (x$type == "customary") {
      ...
      dat$group_color <-
        as.factor(ifelse(dat$Cred.lb < 0 & dat$Cred.ub > 0, 0 , 1))
