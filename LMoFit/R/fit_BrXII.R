...
#'
#' @return A dataframe containing the scale parameter, the shape1 parameter, the shape2 parameter, the squared error of the scale parameter, and the squared error of the shape parameters.
...
#' 
fit_BrXII <- function(sl1, st2, st3) {
  L1s <- sl1; t2s <- st2; t3s <- st3
  conditionx <- con_samlmom_lspace(samplelmom = c(L1s, -99, -99, -99, t2s, t3s, -99), Dist = "BrXII")
  if (conditionx == "lpoint_inside_lspace") {
    suppressWarnings({
      Q <- function(u, scale, shape_1, shape_2) {scale*((((1 - u)^(-1*shape_1*shape_2)) - 1)/shape_2)^(1/shape_1)} #BrXII Distribution
      ...
      opt_res <- stats::optim(par = c(5, 0.5), fn = objective_fn) #default method is Nelder and Mead. good for non-defrentiable function
      ...
      BrXII_par <- data.frame(scale, shape_1, shape_2, se_scale, se_shapes)
      colnames(BrXII_par) <- c("scale", "shape1", "shape2", "se_scale", "se_shapes")
    })
  } else {
    warning(paste0("Consider using another distribution, ", conditionx, " of BrXII distribution. Use com_sam_lspace() to check lmoment ratio diagram"))
    BrXII_par <- data.frame(-99, -99, -99, -99, -99)
    colnames(BrXII_par) <- c("scale", "shape1", "shape2", "se_scale", "se_shapes")
  }
  return(BrXII_par)
}
