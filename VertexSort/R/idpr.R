idpr <- function(vgraph, viteration_no, vparallel=FALSE, vcpus=1) {
  # Calls the Create_rand_g_with_preserving_in_degrees function. Parallel running mode can be enabled.
  # The ultimate goal is to randomize a network by preserving its node in-degrees
  # Args:
  ...
  rand_vgraph <- do.call(list, sfClusterApplyLB(1:viteration_no, Create_rand_g_with_preserving_in_degrees))
