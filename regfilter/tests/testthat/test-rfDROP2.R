...
test_that("the result has the correct class", {
  ...
  data(rock)
  set.seed(9)
  out <- rfDROP2(x = rock[,-ncol(rock)], y = rock[,ncol(rock)])
...
test_that("Invalid k value", {
  ...
  # Default method
  expect_error(rfDROP2(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], k=0))
  # Class formula method
  expect_error(rfDROP2(formula = perm ~ ., data = rock, k=0))
})