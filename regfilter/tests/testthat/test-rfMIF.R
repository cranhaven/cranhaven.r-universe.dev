...
test_that("the result has the correct class", {
  ...
  data(rock)
  set.seed(9)
  out <- rfMIF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)])
...
test_that("Invalid threshold value", {
  ...
  # Default method
  expect_error(rfMIF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], alpha=2))
  # Class formula method
  expect_error(rfMIF(formula = perm ~ ., data = rock, alpha=2))
})
...
test_that("Invalid k value", {
  ...
  # Default method
  expect_error(rfMIF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], k=0))
  # Class formula method
  expect_error(rfMIF(formula = perm ~ ., data = rock, k=0))
})
