...
#'
#' This function lists all the conflicts between packages in the healthyverse
...
healthyverse_conflicts <- function() {
    ...
    tidy_names <- paste0("package:", healthyverse_packages())
    conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

    conflict_funs <- purrr::imap(conflicts, confirm_conflict)
    conflict_funs <- purrr::compact(conflict_funs)

    structure(conflict_funs, class = "healthyverse_conflicts")
}

healthyverse_conflict_message <- function(x) {
    if (length(x) == 0) return("")

    header <- cli::rule(
        left = crayon::bold("Conflicts"),
        right = "healthyverse_conflicts()"
    )
...
#' @export
print.healthyverse_conflicts <- function(x, ..., startup = FALSE) {
    cli::cat_line(healthyverse_conflict_message(x))
}
