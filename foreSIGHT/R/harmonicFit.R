...
get.period.stats <- function(data = NULL, # ts vector for data
  ...
  if (sep.wd == TRUE) { # separate calculations for wet and dry days
    out$wet$m <- rep(NA, nperiod)
    out$wet$sd <- rep(NA, nperiod)
    out$wet$cv <- rep(NA, nperiod)
    if (omit.drypars == FALSE) {
      out$dry$m <- rep(NA, nperiod)
      out$dry$sd <- rep(NA, nperiod)
      out$dry$cv <- rep(NA, nperiod)
    }
    for (h in 1:nperiod) {
    ...
    out$all$m <- rep(NA, nperiod)
    out$all$sd <- rep(NA, nperiod)
    out$all$cv <- rep(NA, nperiod)
    for (h in 1:nperiod) {
...
calc.phase.ang <- function(aa,
  ...
  if (aa > 0) {
    ...
    if (ang < 0) {
      ang <- ang + pi
    } # make positive if less than zero by adding pi
  }
  if (aa < 0) {
    angA <- atan(bb / aa) + pi
    angB <- atan(bb / aa) - pi
